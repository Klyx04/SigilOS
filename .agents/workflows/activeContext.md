...
## ✅ DERNIÈRE PR MERGÉE — **#645** (correctif beta : les 4 sélecteurs de rôles de `Réglages → Marché` étaient **vides**) → `dev` = **`19c347a2e`** — branche courante : **`docs/marche-iteration-finale`** (docs de l'itération finale, PR ouverte vers `dev`)

> **Merges récents :** **#641** (règles projet) · **#642** (S8 lot 3 « Discord & UX ») · **#643** (S8 lot 4 « Supervision & clôture » — **MVP Marché fermé**) · **#644** (clôture : `S3.13` tags de forum + **correctif RBAC bloquant** + carte Discord « tooltip Dofus ») · **#645** (correctif beta : **sélecteurs de rôles vides** en Réglages) → `dev` = **`19c347a2e`**.
> **Session en cours :** **documentation de l'ITÉRATION FINALE du module « Marché »** — branche **`docs/marche-iteration-finale`** depuis `dev` = `19c347a2e` · **plan de travail : `src/temp/refonte-marche/ITERATION-FINALE-MARCHE.md`** (décisions **D-A → D-E** · §2 bugs (`BUG-1`/`BUG-2` corrigés + **§2.3 liste user à compléter**) · §3 tâches **T1 → T7** · §4 ops **O1 → O11** · **§6 conditions de gel** · §7 amorce mot pour mot).
> **Livré (vérifié) :** ✅ **PR #645 mergée** — les **4 sélecteurs de rôles** de `Réglages → Marché` affichaient **aucun rôle** (`market-settings-client.tsx` lisait `rolesRes.data`, propriété inexistante : le contrat est **`{ success, roles }`**) ⇒ `setRoles([])` **systématique** → déballage conforme + `color` entier Discord → **`#rrggbb`** (pastilles colorées) + **texte sous le sélecteur de salon rectifié** (il promettait encore la création automatique des tags de forum, faux depuis **D20**) · garde-fou `tests/unit/market-settings-roles-selectors.test.ts` (**7 tests**) · suite **1168/1168** ✅ · `tsc` **0** · `eslint` **0** · `build` **exit 0** · **0 migration**.
> **État du chantier :** module Marché **fonctionnellement complet** ⇒ **UNE DERNIÈRE ITÉRATION** ouverte, puis **gel** : **bugs repérés en beta par le user** (§2.3 — liste à compléter) + **5 décisions produit** (§1 : *preuves* à retirer ou implémenter · *panneau durées* à replier ou défauts globaux God · **V1.1 = `WANTED` + matching seulement**, le reste écarté · *test flaky* · *`@everyone`*) + **ops** (§4 : **déployer** · **`seed:docs` + `build:seeds`** — *les fiches corrigées ne sont pas encore en base* · **crontab `market-expire`** · DoD visuelle · `S4.12` · Q15 · effets natifs · tags du forum · libellés).
