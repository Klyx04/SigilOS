...
## ✅ DERNIÈRE PR MERGÉE — **#644** (`feat/marche-cloture` : `S3.13` + correctif RBAC + carte Discord) → `dev` = **`4d867ca4e`** — branche courante : **`fix/marche-selecteurs-roles`** (**PR #645** ouverte vers `dev`)

> **Merges récents :** **#640** (lot 2bis) · **#641** (règles projet) · **#642** (S8 lot 3 « Discord & UX ») · **#643** (S8 lot 4 « Supervision & clôture » — **MVP Marché fermé**) · **#644** (clôture : `S3.13` tags de forum + **correctif RBAC bloquant** + carte Discord « tooltip Dofus ») → `dev` = **`4d867ca4e`**.
> **Session en cours :** **correctif beta « aucun rôle Discord dans Réglages → Marché »** — branche **`fix/marche-selecteurs-roles`** depuis `dev` = `4d867ca4e` · commit `dc933c727` · **PR #645** · corps de PR `src/temp/pr-fix-roles-body.md`.
> **Livré (vérifié) :** 🔴 **BUG CORRIGÉ** — `market-settings-client.tsx` déballait la réponse de `getDiscordRolesAction()` via `rolesRes.data` (propriété inexistante : le contrat est **`{ success, roles }`**) ⇒ `setRoles([])` **systématique** ⇒ **aucun rôle** dans les 4 sélecteurs du Marché (constat beta) → déballage conforme + `color` entier Discord → **`#rrggbb`** + **texte sous le sélecteur de salon rectifié** (il promettait encore la création automatique des tags de forum, faux depuis **D20**) · garde-fou `tests/unit/market-settings-roles-selectors.test.ts` · suite **1168/1168** ✅ · `tsc` **0** · `eslint` **0** · `build` **exit 0** · **0 migration**.
> **État du chantier :** le module Marché est **fonctionnellement complet**. Reste **hors code** : ligne crontab `market-expire` sur le VPS (`MAINTENANCE.md:65`), **DoD visuelle** (écrans Marché + `/god/market`, dark/light/mobile), `S4.12` (beta 2 comptes), **`seed:docs`** sur l'env déployé (les fiches `marche`/`admin-marche` corrigées ne sont pas encore en base), et **3 décisions produit** : (1) **preuves/médias** — interrupteur **sans effet** (aucun upload n'existe) ⇒ implémenter ou retirer ; (2) **panneau « Durées, plafonds & rappels »** — valeurs **déjà pré-remplies** par les défauts ⇒ repli « avancé » ou défauts globaux God ; (3) **V1.1 (`S6`)** — le user ne retient que **`WANTED` + matching** (« le reste non »).
