...
## ✅ DERNIÈRE PR MERGÉE — **#646** (docs : ouverture de l'itération finale du module Marché) → `dev` = **`39e748724`** — branche courante : **`docs/marche-debug-beta`** (recette beta : **5 décisions ratifiées + 9 bugs diagnostiqués**)

> **Merges récents :** **#642** (S8 lot 3 « Discord & UX ») · **#643** (S8 lot 4 « Supervision & clôture » — **MVP Marché fermé**) · **#644** (clôture : `S3.13` tags de forum + **correctif RBAC bloquant** + carte Discord « tooltip Dofus ») · **#645** (correctif beta : **sélecteurs de rôles vides** en Réglages) · **#646** (docs : ouverture de l'itération finale) → `dev` = **`39e748724`**.
> **Session en cours :** **recette beta du Marché** (verbatim user `src/temp/debug.md`) — branche **`docs/marche-debug-beta`** · **backlog complet : `src/temp/refonte-marche/ITERATION-FINALE-MARCHE.md`** (**§1** décisions ratifiées · **§2.3** bugs `BUG-3`→`BUG-11` avec causes vérifiées · **§2.4** spec carte Discord « tout dans l'image » · **§3** tâches **T8→T15** + ordre · **§4** ops · **§6** conditions de gel · **§7** amorce mot pour mot).
> **Livré (vérifié) :** ✅ **#645** (sélecteurs de rôles : le panneau lisait `rolesRes.data`, contrat réel `{ success, roles }`) · ✅ **#646** (docs itération finale) · 📌 **5 décisions RATIFIÉES par le user** (13/09 soir) : **preuves retirées** (l'interrupteur était **sans effet**) · **durées/plafonds = défauts globaux posés par le God** · **`@everyone` interdit** dans les rôles pinguables · **test flaky** `lot4-slash-commands-api-keys` → **déterministe** (horloge injectée, jamais désactivé) · **V1.1 = `WANTED` + matching SEULEMENT** (hors itération) ⇒ inscrites en **§2.1 D44→D48** du plan.
> **État du chantier :** module Marché **fonctionnellement complet**, **DERNIÈRE ITÉRATION** en cours (puis **gel**). **9 bugs beta à corriger** (🔴 images d'objets **404/relatives** · **6 assets de stats manquants** · **carte Discord incomplète** ⇒ **tout dans l'image** + prix & **icône Kamas** · **filtres/familles** de `/marche/nouveau` ; 🟠 « Faire une offre » = **faux** « termine ton profil » · **supprimer** « Contacter » · réservation : **« Me désister »** + re-clic + **notif vendeur** + **rate-limit** · `React #418` hydratation ; 🟡 CSS `text/plain`). **Ops** : déployer · **`seed:docs` + `build:seeds`** (*fiches pas encore en base*) · **crontab `market-expire`** · DoD visuelle · `S4.12` · Q15 · effets natifs · tags du forum · libellés (`S1.25`).
