...
## ✅ DERNIÈRE PR MERGÉE — **#657** (fix Marché : **embeds & cartes selon la famille**) → `dev` = **`e54f332cc`** — branche `fix/marche-embeds-et-cartes-familles` (commits `2e987985e` + `a89bd9e8e`, **8 fichiers**, +365 / −47 ; CI **verte** : `Verify & Build` **7 m 45 s** · CodeQL · Analyze · `Filter changed files` ; hook pre-commit vert ; **0 migration** ; branche supprimée)
...
> **Correctifs** : **`pickMarketEmbedImage()`** (pur) ⇒ carte PNG **seulement** s'il y a un **jet réel** (`isStatBearingStatRow`), sinon **image de l'objet en grand** (les **deux** points d'entrée Discord) · carte OG = **toutes** les lignes, **`statRowMetrics()`** (compression par paliers >12/>18/>24), **hauteur dynamique**, **image du 1ᵉʳ composant** pour un lot · icône de lot au **catalogue** (carte + tableau) et dans **« Mon espace »** · `MarketItemCard` **compacte** sans jet (vignette 80 px centrée, textes agrandis).
...
> ⚠️ **Décisions ouvertes** : « Réserver au prix » pendant une réservation (file d'attente ?) · assets `C:\Users\user\Desktop\dofus_assets` à **proposer puis valider** avant intégration.
> 🔎 Résidus connus : 4 `<Image>` **bruts** sans repli `onError` dans l'assistant de création (`market-create-client.tsx`, URLs déjà normalisées).
