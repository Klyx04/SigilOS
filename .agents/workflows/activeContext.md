...
> **Merges récents :** **#640** (lot 2bis) · **#641** (règles projet) · **#642** (S8 lot 3 « Discord & UX ») · **#643** (S8 lot 4 « Supervision & clôture » — **MVP Marché fermé**) → `dev` = **`c734b6f9e`**.
> **Session en cours :** **Module « Marché » · clôture** (`S3.13` tags de forum + **correctif RBAC bloquant** + **carte Discord « tooltip Dofus »**) — branche **`feat/marche-cloture`** depuis `dev` = `c734b6f9e` · mémo `src/temp/memo-2026-09-13-marche-cloture.md`.
> **Livré (vérifié) :** 🔴 **BUG CORRIGÉ** — `getUserContext()` ne lisait pas `marche` dans `modules` (ni `reactionRoles`/`tickets`/`commandes`/`admin`) ⇒ `applyModule(false, perm)` bloquait **tout le monde sauf le God** (module activé en base mais jamais lu : entrée sidebar absente + « Accès Restreint ») → **5 clés ajoutées au `select`** + test de non-régression · **`S3.13`** tags de forum Discord (`src/lib/market/forum-tags.ts` pur + `updateForumThreadTags` + `appliedTags` à la publication/changement de statut + section « Tags de forum » en Réglages, ids revalidés contre `available_tags`) · **carte Discord refondue** (`/api/og/market/[id]`) : image de l'objet à droite, **effets à gauche avec les assets officiels**, couleurs over/malus/exo/transcendance + badges · doc `admin-marche` **corrigée** (aucun tag créé automatiquement) · suite **1161/1161** ✅ · `tsc` **0** · `eslint` **0 erreur** · `build` **exit 0** · **0 migration**.
> **État du chantier :** le module Marché est **fonctionnellement complet**. Reste **hors code** : ligne crontab `market-expire` sur le VPS (`MAINTENANCE.md:65`), **DoD visuelle** (écrans Marché + `/god/market`, dark/light/mobile), `S4.12` (beta 2 comptes), et la **décision V1.1** (`S6` : WANTED/matching, SERVICE, réputation, page SEO publique, import Dofusbook).
