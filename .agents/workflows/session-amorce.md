...
>
> **Règles de référence (toujours valables)** : `RULES.md` · `SECURITY.md` ·
> `.agents/workflows/{git-push,prisma-schema-change,add-cron-task,discord-module,dev-local}.md`.
...
- **0 migration** = l'idéal. Si le schéma change : `npx prisma migrate dev --name description_courte`, migration **committée avec** `schema.prisma`, **additive** et **idempotente** (`ADD COLUMN IF NOT EXISTS`), **jamais** de reset destructif, `npx prisma validate` + `migrate diff` **sans écart** (cf. `prisma-schema-change.md`). **Prévenir le user AVANT.**

### 5.2 CRON (nouvelle tâche) — `add-cron-task.md`
`verifyCronSecret(req)` **fail-closed** · `recordCronExecution(id, { success, durationMs, summary, details })` · tâche déclarée dans **`KNOWN_CRON_TASKS`** (`src/lib/cron-telemetry.ts`) ⇒ visible dans `/god?tab=cron-status` · ligne crontab VPS **documentée dans `MAINTENANCE.md`** · test unitaire + appel `curl` local.

### 5.3 Discord — `discord-module.md`
...
### 5.5 Documentation du module (obligatoire — `RULES.md` § doc)
...
## 6. 🌿 Git / CI-CD (rappel `git-push.md`)
...
docs-catalog.ts a changé. Push : .agents/workflows/git-push.md (sync origin/dev +
