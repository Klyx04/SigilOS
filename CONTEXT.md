...
- **Scalabilité module guide ✅ P0+P1 (13/08, commits `100f947a` + `3dce90c9` + `f938d2ed` + `96d1f002`)** : **cache Redis 3s des agrégats** `getGuildOptimizedGuideProgress` (clé `guide:progress:{guildId}:{slug}`, invalidation sur chaque write) — plus de re-fetch de ~10k lignes à chaque page view · **rate-limit des mutations membres** (toggle 30/min, step 120/min, reset, complete, bookmark) + **validation `completedSteps`** · **WS présence sans requête DB** (identité résolue au `guide:join`, stockée sur le socket) · **rate-limit admin God** (`requireGuideWriteAccess`). Reste (P2) : lazy-load admin `god/dofus-guides`, `console.error`→logger, normalisation `completedSteps`.
