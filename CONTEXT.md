...
> **Statut (19/08, 2e passe)** : ✅ Lot 0 (socle) + ✅ Lot 1 (visibilité) + ✅ UI admin (bouton Enregistrer sticky) + ✅ **interface God** (kill-switch global + plafonds par module) + ✅ **sidebar redimensionnable persistée** + ✅ **seed guildes locales** (`scripts/seed-inter-guilde-demo.ts` : G1/G2 Draconiros + G3 Imagiro, 24 membres). ⚪ Lots 2-5 à venir (calendrier raids `openToExternals`, galerie/mini-jeux GLOBAL, ladder/ocre/songes/donjons/services/quetes, God canaux Discord cross-guildes + tours tutoriels).
> **Concept** : une guilde peut s'ouvrir aux autres guildes SigilOS — **opt-in bilatéral fail-closed**, par défaut **même serveur Dofus** (`dofusServerId`), `gallery`/`minigames` en `GLOBAL`. Master switch par guilde (`/admin/settings` → Inter-Guilde) + réglage par module. **God (interface FAITE 19/08)** : kill-switch global `interGuildGlobalEnabled` + plafonds par module (le plus restrictif gagne) ; canaux Discord cross-guildes anticipés dans le schéma (Lot 5). **RBAC** : la permission du module est vérifiée dans la guilde d'origine du membre (aucune élévation). **PII minimale** : pseudo Dofus + tag guilde, jamais d'identité Discord. Migration `20260819000000_add_inter_guild`.
