...
### ⚠️ Restant / à savoir
- **Branche `refonte-module-ganymede`** : 15 commits (session 12/08) prêts → **PR vers `dev`** (auto-approbation solo). `dev` étant protégé, le merge passe par la PR GitHub.
- **Reste à faire (mémo `src/temp/memo-2026-08-12-refonte-ganymede-rush.md`)** : **G** portages UX croisés (recherche persistante, ContextualHelp, verrouillage prérequis Rush→Ganymède ; CoordHoverMap→Rush) · **I** agrégation serveur `getGuildOptimizedGuideProgress` (payload brut → select chirurgical + présenceMap côté serveur) · **J** finalisation (console.error page.tsx → logger, recette UX) · **Bonus** : facepile Rush (RushTimelineClient) temps réel.
