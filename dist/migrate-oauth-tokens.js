...
var createPrismaClient = () => {
  ...
  const pool = new import_pg3.Pool({
    connectionString,
    user: poolUser || void 0,
    password: poolPassword || void 0,
    max: isDev ? 8 : 30,
...
async function migrate() {
  console.log(`[Migration] ${DRY_RUN ? "DRY-RUN (aucune modification)" : "RUN R\xC9EL"} \u2014 chiffrement des jetons OAuth en clair`);
  ...
  for (const acc of accounts) {
    const update = { id: acc.id };
    if (acc.access_token) {
      const at = encryptIfNeeded(acc.access_token);
      if (at !== acc.access_token) {
    ...
    if (update.access_token !== void 0 || update.refresh_token !== void 0 || update.id_token !== void 0) {
      fieldsByAccount.push(update);
    }
  }
  console.log(`[Migration] ${accounts.length} compte(s) Discord avec jetons.`);
  console.log(`[Migration] Jetons d\xE9j\xE0 chiffr\xE9s : ${alreadyEncrypted}`);
  console.log(`[Migration] Jetons \xC0 CHIFFRER : ${toEncryptCount} (r\xE9partis sur ${fieldsByAccount.length} compte(s))`);
  if (DRY_RUN) {
    console.log("[Migration] DRY-RUN termin\xE9 \u2014 AUCUNE modification effectu\xE9e.");
    await db.$disconnect();
    return;
  }
  let updated = 0;
  ...
  console.log(`[Migration] \u2705 ${updated} compte(s) mis \xE0 jour avec des jetons chiffr\xE9s.`);
  await db.$disconnect();
}
