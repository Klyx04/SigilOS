...
// src/workers/metamob-worker.ts
var import_bullmq2 = require("bullmq");

// src/lib/queue/metamob-queue.ts
...
var redis = globalForRedis.redis ?? new import_ioredis.default(redisUrl, {
...
// src/lib/queue/metamob-queue.ts
var METAMOB_QUEUE_NAME = "metamob-sync";
...
var globalForQueue = global;
var metamobQueue = globalForQueue.metamobQueue ?? new import_bullmq.Queue(METAMOB_QUEUE_NAME, defaultQueueOptions);
if (process.env.NODE_ENV !== "production") {
...
function getEncryptionKey() {
  ...
  if (!keyStr) {
    ...
    return (0, import_crypto.scryptSync)("dev-fallback-sigilos-key-2026", "salt", KEY_LENGTH);
  }
  if (keyStr.length === 64) {
    return Buffer.from(keyStr, "hex");
  }
  return (0, import_crypto.scryptSync)(keyStr, "sigilos-salt", KEY_LENGTH);
}
...
// src/lib/prisma.ts
var getEnv2 = (key, fallback) => {
  const val = process.env[key];
  if (!val) return fallback;
  return val.replace(/^['"]|['"]$/g, "").trim();
};
var user = getEnv2("POSTGRES_USER", "");
var pwd = getEnv2("POSTGRES_PASSWORD", "");
var db_name = getEnv2("POSTGRES_DB", "");
var isBeta = process.env.DOMAIN_NAME?.includes("beta") || process.env.NEXT_PUBLIC_APP_URL?.includes("beta");
var defaultHost = isBeta ? "db-beta" : "db-prod";
var host = process.env.DB_HOST || (process.env.NODE_ENV === "production" ? defaultHost : "localhost");
var dbUrl = getEnv2("DATABASE_URL", "");
var protocol = "postgresql://";
var safeFromComponents = `${protocol}${encodeURIComponent(user)}:${encodeURIComponent(pwd)}@${host}:5432/${db_name}?schema=public`;
var connectionString = user && pwd ? safeFromComponents : dbUrl || safeFromComponents;
var createPrismaClient = () => {
  const pool = new Pool({
    connectionString,
    max: 30,
    // Increased pool size for high concurrency (2026)
    idleTimeoutMillis: 15e3,
    // Faster cleanup of idle connections
    connectionTimeoutMillis: 5e3
    // Fail fast if DB is slow
  });
  const adapter = new PrismaPgAdapterFactory(pool);
  const basePrisma = new import_client.PrismaClient({
    adapter,
    log: ["error", "warn"]
  });
...
// src/lib/metamob-client.ts
var METAMOB_API_BASE = "https://www.metamob.fr/api";
var LocalizedNameSchema = external_exports.object({
  fr: external_exports.string(),
  en: external_exports.string(),
  es: external_exports.string()
});
...
var GameVersionSchema = external_exports.object({
...
var ServerSchema = external_exports.object({
  id: external_exports.number(),
  name: external_exports.string(),
  community: external_exports.string(),
  game_version: GameVersionSchema
});
var AvatarSchema = external_exports.object({
  id: external_exports.number(),
  name: LocalizedNameSchema,
  image: external_exports.string()
});
var MonsterTypeSchema = external_exports.object({
  id: external_exports.number(),
  name: LocalizedNameSchema
});
var MonsterSchema = external_exports.object({
  id: external_exports.number(),
  name: LocalizedNameSchema,
  image: external_exports.string(),
  level_min: external_exports.number(),
  level_max: external_exports.number(),
  type: MonsterTypeSchema,
  reference: external_exports.object({
    id: external_exports.number(),
    name: LocalizedNameSchema
  }).optional(),
  zones: external_exports.array(external_exports.object({
    id: external_exports.number(),
    name: LocalizedNameSchema,
    subzones: external_exports.array(external_exports.object({
      id: external_exports.number(),
      name: LocalizedNameSchema
    }))
  })).optional()
});
var QuestMonsterSchema = external_exports.object({
  id: external_exports.coerce.number(),
  monster_id: external_exports.coerce.number().optional(),
  name: LocalizedNameSchema,
  image: external_exports.string().optional().nullable(),
  level_min: external_exports.coerce.number().optional().nullable(),
  level_max: external_exports.coerce.number().optional().nullable(),
  type: MonsterTypeSchema.optional().nullable(),
  step: external_exports.coerce.number().optional().nullable(),
  owned: external_exports.coerce.number().optional().nullable(),
  amount: external_exports.coerce.number().optional().nullable(),
  quantity: external_exports.coerce.number().optional().nullable(),
  quantite: external_exports.coerce.number().optional().nullable(),
  status: external_exports.coerce.number().optional().nullable(),
  want: external_exports.coerce.number().optional().nullable(),
  offer: external_exports.coerce.number().optional().nullable(),
  reference: external_exports.object({
    id: external_exports.number(),
    name: LocalizedNameSchema
  }).optional(),
  monster: external_exports.object({
    id: external_exports.coerce.number(),
    owned: external_exports.coerce.number().optional().nullable(),
    quantity: external_exports.coerce.number().optional().nullable(),
    quantite: external_exports.coerce.number().optional().nullable(),
...
var QuestTemplateSchema = external_exports.object({
  id: external_exports.number(),
  monster_count: external_exports.number().optional(),
  step_count: external_exports.number().optional(),
  game_version: GameVersionSchema.optional()
});
var UserQuestSchema = external_exports.object({
  slug: external_exports.string(),
  character_name: external_exports.string(),
  current_step: external_exports.number(),
  parallel_quests: external_exports.number(),
  wanted_count: external_exports.number().optional(),
  offered_count: external_exports.number().optional(),
  server: ServerSchema,
  quest_template: QuestTemplateSchema
});
var QuestTemplateDetailsSchema = QuestTemplateSchema.extend({
  monsters: external_exports.array(QuestMonsterSchema),
  pagination: PaginationSchema
});
var UserProfileSchema = external_exports.object({
  username: external_exports.string(),
  bio: external_exports.string().nullable().optional(),
  avatar: AvatarSchema.nullable().optional(),
  created_at: external_exports.string().optional(),
  last_active: external_exports.string().optional(),
  quests: external_exports.array(UserQuestSchema).optional()
});
var QuestDetailsSchema = external_exports.object({
  slug: external_exports.string(),
  character_name: external_exports.string(),
  current_step: external_exports.number(),
  parallel_quests: external_exports.number(),
...
var MatchMonsterSchema = external_exports.object({
  id: external_exports.number(),
  name: LocalizedNameSchema,
  available: external_exports.number(),
  needed: external_exports.number(),
  covers_need: external_exports.boolean()
});
var MatchPartnerSchema = external_exports.object({
  user: external_exports.object({
    username: external_exports.string(),
    avatar: AvatarSchema.nullable().optional(),
    last_active: external_exports.string().optional()
  }),
  quest: external_exports.object({
    slug: external_exports.string(),
    character_name: external_exports.string(),
    parallel_quests: external_exports.number()
  }),
  matches: external_exports.object({
    they_have_you_want: external_exports.array(MatchMonsterSchema),
    you_have_they_want: external_exports.array(MatchMonsterSchema)
  }),
  match_score: external_exports.number()
});
...
var KralamoureEventSchema = external_exports.object({
  id: external_exports.number(),
  event_datetime: external_exports.string(),
  description: external_exports.string().nullable().optional(),
  creator: external_exports.string(),
  participants_count: external_exports.number().optional(),
  character_count: external_exports.number().optional(),
  messages_count: external_exports.number().optional(),
  server: ServerSchema
});
var KralamoureParticipantSchema = external_exports.object({
  username: external_exports.string(),
  character_count: external_exports.number()
});
var KralamoureEventDetailsSchema = KralamoureEventSchema.extend({
  participants: external_exports.array(KralamoureParticipantSchema).optional(),
...
var MetamobApiError = class extends Error {
  constructor(code, message) {
    super(message);
    this.code = code;
  }
};
async function fetchApi(endpoint, schema, options = {}) {
  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      ...
      if (apiKey) {
        if (isUserResource) {
          throw new MetamobApiError("UNAUTHORIZED", "Acc\xE8s refus\xE9 : Ce compte Metamob est priv\xE9 ou la cl\xE9 API est invalide.");
        }
        ...
        if (retryResponse.ok) {
          const json2 = await retryResponse.json();
          const data2 = json2.data !== void 0 ? json2.data : json2;
          return schema.parse(data2);
        }
        if (retryResponse.status === 401 || retryResponse.status === 403) {
          throw new MetamobApiError("UNAUTHORIZED", "Acc\xE8s refus\xE9 : Une cl\xE9 API valide est requise pour cette ressource.");
        }
        throw new MetamobApiError("INVALID_API_KEY", "Cl\xE9 API Metamob invalide ou expir\xE9e.");
      } else {
        throw new MetamobApiError("UNAUTHORIZED", "Acc\xE8s refus\xE9 : Authentification requise.");
      }
    }
    ...
    if (error instanceof external_exports.ZodError) {
      ...
      throw new MetamobApiError("API_ERROR", "Format de r\xE9ponse API invalide");
    }
    throw error;
  }
}
async function getQuestDetails(username, slug, options) {
...
// src/workers/metamob-worker.ts
logger.info(`[Worker] D\xE9marrage du Background Worker pour la queue: ${METAMOB_QUEUE_NAME}...`);
async function processExchangeJob(job) {
  ...
  if (!currentUserProfile?.metamobQuestSlug || !currentUserProfile.metamobVerified) {
    throw new Error("Aucune qu\xEAte Metamob li\xE9e pour cet utilisateur.");
  }
  const effectiveApiKey = decrypt(currentUserProfile.metamobApiKey) || decrypt(currentUserProfile.guild?.metamobApiKey) || void 0;
  ...
  const neededMonsterIds = /* @__PURE__ */ new Set();
  const processUserMonsters = (monsters) => {
    for (const m of monsters) {
      const rawOwned = m.owned ?? m.quantite ?? m.amount ?? m.quantity ?? 0;
      if (rawOwned < currentUserPQ) neededMonsterIds.add(m.id);
    }
  };
  ...
  const guildMembers = await prisma.userProfile.findMany({
    where: {
  ...
  for (let i = 0; i < guildMembers.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (member) => {
      ...
      try {
        ...
        const monstersTheyHave = allMonsters.filter((m) => {
          let available = 0;
          if (typeof m.offer === "number") available = m.offer;
          else {
            const rawOwned = m.owned ?? m.quantite ?? m.amount ?? m.quantity ?? 0;
          ...
          const coversNeed = neededMonsterIds.has(m.id);
          return {
            id: m.id,
            name: m.name?.fr || "Inconnu",
            imageUrl: m.image ? `https://www.metamob.fr/img/monsters/${m.image}` : void 0,
            available,
        ...
        if (monstersTheyHave.length > 0) {
          ...
          partners.push({
            ...
            monstersYouHave: [],
            // We don't cross-calculate "monsters you have that they need" for now to save API calls
            matchScore
          });
        }
      } catch (memberError) {
  ...
  logger.info(`[Worker] Job ${job.id} termin\xE9 avec ${partners.length} partenaires trouv\xE9s.`);
  return partners;
}
var worker = new import_bullmq2.Worker(METAMOB_QUEUE_NAME, processExchangeJob, {
  ...defaultQueueOptions,
  concurrency: 2
  // Process up to 2 syncs at the exact same time
});
worker.on("completed", (job) => {
  logger.info(`[Worker] \u2705 Job ${job.id} compl\xE9t\xE9 avec succ\xE8s.`);
});
worker.on("failed", (job, err) => {
  logger.error(`[Worker] \u274C Job ${job?.id} a \xE9chou\xE9: ${err.message}`);
});
var shutdown = async () => {
