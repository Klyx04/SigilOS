...
| Outil | Cité par |
|---|---|
| `scripts/probe-figure-slots.mjs` | `MAINTENANCE.md` §figures landing · `src/lib/landing-figures.ts` |
| `scripts/crop-landing-visuels.mjs` | `MAINTENANCE.md` · `docs/DECISION-OUVERTURE-LANDING.md` · `src/lib/landing-figures.ts` |
| `scripts/scan-mojibake-all.mjs` | `CONTEXT.md` §Encodage (règle dure : doit afficher **0 fichier**) |
| `scripts/fix-mojibake.mjs` | `CONTEXT.md` §Encodage (réparation) |
...
- `chantier-actif.md` — demandes **ouvertes** de la session en cours (lu au démarrage, cf. `PROMPT_START.md`).
- `debug.md` — retours en cours de session (cité par `.agents/workflows/session-amorce.md`).
...
**Conservé** : `scripts/capture-landing-visuels.mjs` (cité comme producteur des visuels par `MAINTENANCE.md`
