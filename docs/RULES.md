...
|---|---|---|
| **Épingler toute action sur un SHA de commit complet** (`owner/action@<sha40> # vX`) — **first-party `actions/*` comprise** | un tag mobile peut être redéplacé sur du code malveillant (supply chain) ; `deploy.yml` porte `packages: write` (une action détournée fabrique l'**image de production**) | ✅ **les 14 `uses:` des 2 workflows** (20/09/2026, F2) · garde automatique `tests/unit/workflows-actions-pinned.test.ts` (pin 40 hex **+** tag en commentaire, forme suivie par Dependabot) · vérif manuelle : `grep -nE 'uses: [^ ]+@v[0-9]' .github/workflows/*.yml` → **0** |
| **Images de production avec provenance + SBOM** (`--provenance=true --sbom=true` sur chaque `buildx build`) | permet de vérifier **d'où vient** l'image déployée (chaîne de fabrication attestée, pas seulement un tag) | `deploy.yml` (fonction `retry_push` : les 4 images app/worker/ws/bot) — verrouillé par le test ci-dessus ; `--provenance=true` = mode **minimal** (mode `max` embarquerait les `--build-arg`) |
