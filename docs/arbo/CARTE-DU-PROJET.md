...
| **Assets & captures d'écran** (~8) | `capture-screenshots.mjs`, `capture-landing-visuels.mjs`, `probe-figure-slots.mjs`, `crop-landing-visuels.mjs`, `scan-mojibake-all.mjs`, `fix-mojibake.mjs`, `sync-assets.sh/.ps1` | ✅ **vivants** (cités par `docs/MAINTENANCE.md` / `docs/CONTEXT.md`) |
