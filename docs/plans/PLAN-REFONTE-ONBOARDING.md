...
- Résolution dans `getGuildModules` (`module-actions.ts`) : God-disabled ⇒ `false` quel que soit le toggle guilde. Le `applyModule` existant (`user-actions.ts`) propage déjà le `false` à tous les `canView*/canManage*` → pages + cartes disparaissent **sans autre changement**. Invalider `moduleCache` + `flushGuildUserContextCache` (pattern existant).
- UI God (page détail `/god/guilds/[id]`, section Modules) : grille des modules avec état effectif (toggle guilde × lock God) + lock/unlock + audit (`createAuditLog`, `isGodLog: true`) + notif salon staff si configuré.
