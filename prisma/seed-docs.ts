...
async function main() {
    console.log('🌱 Seeding documentation officielle SigilOS...');
    
    // Purge des anciennes documentations orphelines / placeholders obsolètes
    const validSlugs = docs.map(d => d.slug);
    const deleted = await prisma.docPage.deleteMany({
        where: {
    ...
    console.log(`🎉 Seeding completed (${docs.length} documentations enregistrées).`);
}
