...
function fetchDofusDbPage(type, skip) {
    return new Promise((resolve) => {
        let raw = '';
        const url = 'https://api.dofusdb.fr/' + type + '?%24limit=50&%24skip=' + skip + '&%24select=name.fr';
        https.get(url, (res) => {
...
async function fetchAllResources(type, categoryName) {
    ...
    while(true) {
        ...
        if (items.length < 50) break; // Finished pagination!
    }
    console.log('Fetched ' + categoryName + ' (endpoint ' + type + '): ' + totalItems + ' entités réelles siphonnées.');
}
...
async function main() {
    ...
    await fetchAllResources('monsters', 'Monstre');
    ...
    console.log('Succès ! Dictionnaire absolu Dofus : ' + Object.keys(dofusWords).length + ' mots/combinaisons uniques sauvées !');
}
