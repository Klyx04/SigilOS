...
var pwd = getEnv("POSTGRES_PASSWORD", "");
var db_name = getEnv("POSTGRES_DB", "");
...
var port = process.env.DB_PORT || (process.env.NODE_ENV === "production" ? "5432" : "5433");
var protocol = "postgresql://";
var safeFromComponents = `${protocol}${encodeURIComponent(user)}:${encodeURIComponent(pwd)}@${host}:${port}/${db_name}?schema=public`;
var connectionString = dbUrl || safeFromComponents;
var createPrismaClient = () => {
  const isDev = process.env.NODE_ENV !== "production";
  const pool = new Pool({
    connectionString,
    max: isDev ? 8 : 30,
    // Reduced from 10 to prevent exhaustion in local HMR
    idleTimeoutMillis: 5e3,
    // Reduced from 30s to release connections faster
    connectionTimeoutMillis: 5e3,
    // Fail fast (5s) instead of hanging (15s)
    ...
    statement_timeout: isDev ? 3e4 : 1e4,
    query_timeout: isDev ? 3e4 : 1e4,
...
// scripts/siphon-guide-images.ts
var import_fs = __toESM(require("fs"));
var import_path = __toESM(require("path"));
var import_crypto2 = __toESM(require("crypto"));
var import_sharp = __toESM(require("sharp"));
var TARGET_HOSTS = /* @__PURE__ */ new Set([
  "i.imgur.com",
  "imgur.com",
  "i.gyazo.com",
...
var UPLOADS_DIR = import_path.default.join(process.cwd(), "public/uploads/guides");
async function main() {
  console.log("\u{1F680} Initialisation du siphonneur d'images de guides...");
  if (!import_fs.default.existsSync(UPLOADS_DIR)) {
    import_fs.default.mkdirSync(UPLOADS_DIR, { recursive: true });
    console.log(`\u{1F4C1} Dossier cr\xE9\xE9 : ${UPLOADS_DIR}`);
  }
  console.log("\u{1F4DA} R\xE9cup\xE9ration des guides en base de donn\xE9es...");
  const guides = await prisma.subGuideData.findMany();
  console.log(`\u2705 ${guides.length} guides r\xE9cup\xE9r\xE9s.`);
  const imgRegex = /<img[^>]+src=["']?([^"']+)["']?/gi;
  const urlToLocalMap = /* @__PURE__ */ new Map();
  const urlsToDownload = /* @__PURE__ */ new Set();
    ...
    for (const step of stepsArray) {
      ...
      while ((match = imgRegex.exec(text)) !== null) {
        ...
        try {
          ...
          if (TARGET_HOSTS.has(host2)) {
            if ((host2 === "ganymede-dofus.com" || host2 === "ganymede-app.com") && !src.includes("guides.png")) {
              continue;
            }
            const hash = import_crypto2.default.createHash("md5").update(src).digest("hex");
  ...
  console.log(`\u{1F4E6} Trouv\xE9 ${urlsToDownload.size} images uniques cibles \xE0 siphonner.`);
  const urlList = Array.from(urlsToDownload);
  let downloadedCount = 0;
  let skippedCount = 0;
  let failedCount = 0;
  console.log("\u26A1 D\xE9marrage du siphonnage s\xE9quentiel avec retry (Imgur rate-limit protection)...");
  async function downloadWithRetry(url, localPath, maxRetries = 4) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        const baseDelay = attempt === 0 ? 1500 : 2e3 * Math.pow(2, attempt - 1);
        await new Promise((resolve) => setTimeout(resolve, baseDelay + Math.random() * 1e3));
        ...
        if (res.status === 429) {
          const waitMs = 5e3 * Math.pow(2, attempt);
          console.log(`  \u23F3 Rate limit (429) pour ${url}. Attente ${waitMs / 1e3}s avant retry ${attempt + 1}/${maxRetries}...`);
        ...
        await (0, import_sharp.default)(buffer).webp({ quality: 65, effort: 6 }).toFile(localPath);
        return true;
      } catch (err) {
        if (attempt === maxRetries) {
          console.error(`  \u274C \xC9chec d\xE9finitif pour ${url}: ${err.message}`);
          return false;
        }
        console.error(`  \u26A0\uFE0F  Tentative ${attempt + 1} \xE9chou\xE9e pour ${url}: ${err.message}`);
      }
    }
    return false;
  }
  for (const url of urlList) {
    const localPath = import_path.default.join(UPLOADS_DIR, import_path.default.basename(urlToLocalMap.get(url)));
    ...
    if (success) {
      ...
      if (downloadedCount % 10 === 0 || downloadedCount === 1) {
        console.log(`  \u2705 [${downloadedCount}/${urlList.length}] images t\xE9l\xE9charg\xE9es...`);
      }
    } else {
      failedCount++;
    }
  }
  console.log("\n\u{1F4CA} Bilan du siphonnage :");
  console.log(`- T\xE9l\xE9charg\xE9es & compress\xE9es : ${downloadedCount}`);
  console.log(`- D\xE9j\xE0 pr\xE9sentes (pass\xE9es)   : ${skippedCount}`);
  console.log(`- \xC9checs                     : ${failedCount}`);
  console.log("\n\u{1F504} Mise \xE0 jour de la base de donn\xE9es...");
    ...
    const updatedSteps = stepsArray.map((step) => {
      ...
      while ((match = imgRegex.exec(stepText)) !== null) {
        ...
        if (urlToLocalMap.has(src)) {
          ...
          const localFilename = import_path.default.basename(localRelPath);
          const fullLocalPath = import_path.default.join(UPLOADS_DIR, localFilename);
          if (import_fs.default.existsSync(fullLocalPath)) {
  ...
  console.log(`\u2705 Base de donn\xE9es mise \xE0 jour !`);
  console.log(`- Guides modifi\xE9s en DB : ${updatedGuidesCount}`);
  console.log(`- URL d'images remplac\xE9es dans les \xE9tapes : ${totalReplacementsCount}`);
  await prisma.$disconnect();
  console.log("\u2728 Op\xE9ration de siphonnage et compression termin\xE9e avec succ\xE8s !");
}
