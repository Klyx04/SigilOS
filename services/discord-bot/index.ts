...
// VoiceUser — même forme que l'ancien DiscordVoiceService (rétro-compat frontend).
interface VoiceUser {
    userId: string;
    userName: string;
    avatar: string | null;
...
const voiceRefreshTimers = new Map<string, NodeJS.Timeout>();
const VOICE_REFRESH_DEBOUNCE_MS = 500;

/** Reconstruit l'état voix d'une guilde puis le publie (pub/sub) + le stocke (snapshot). */
async function refreshAndPublishVoice(guildId: string): Promise<void> {
...
function queueVoiceRefresh(guildId: string): void {
...
client.on(Events.VoiceStateUpdate, (oldState, newState) => {
    ...
    if (guildId) queueVoiceRefresh(guildId);
});
