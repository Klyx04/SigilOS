...
> **📌 02/09 — Overlay Rush Sylvestre TRÈS MATURE + correctifs dashboard** (branche `fix/cron-maintenance-scripts` → PR `dev`) : l'overlay (`GuideOverlayClient` + `components/RushOverlay*`) couvre recherche globale, arbre de chapitres `<select>`, modale Ressources (Restantes/Toutes), modale Membres, tutoriel, mode compact, header 7 boutons + puce personnage, validation chapitre fiable, blocs non cochables, `goToNextMs`, exclusivité validé/repère, bouton bug pré-rempli, **synchro dashboard↔overlay** (`BroadcastChannel`). **Fix bug « repère disparaît après 2 s »** (dashboard ne lisait que `seq:`-préfixé). **Reste dashboard** : modale Ressources globale + Restantes/Toutes · contexte d'étape dans « Signaler » · puce personnage Main/Mule · suppression `RushOverlayQuestPanel.tsx` orphelin · ressources `kind:"unresolved"` (données).
