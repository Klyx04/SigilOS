...
export function AlmanaxWidget({ 
    ...
    return (
        <Card className="glass-premium relative overflow-hidden group border-border hover:border-warning/20 transition-all min-h-[380px] flex flex-col">
            ...
            <CardContent className="space-y-6 flex-1 flex flex-col justify-center">
                {almanax ? (
                    <div className="space-y-6 animate-in fade-in duration-300">
                        ...
                        <div className="flex items-start gap-4 p-4 rounded-2xl bg-foreground/[0.02] border border-border group-hover:bg-foreground/[0.04] transition-colors">
                            <div className="relative w-16 h-16 bg-background rounded-xl border border-border flex items-center justify-center overflow-hidden shrink-0 shadow-2xl">
                                <NextImage 
                                    ...
                                    unoptimized
                                />
                                <div className="absolute -bottom-1 -right-1 bg-warning text-warning-foreground text-caption font-black px-1.5 rounded-sm shadow-xl">
                            ...
                            <div className="space-y-1">
                                ...
                                {almanax.reward_kamas && (
                                    <div className="text-caption font-bold text-warning text-warning/80">+{almanax.reward_kamas.toLocaleString()} kamas</div>
                                )}
                            </div>
                        </div>
                        
                        {/* Bonus Card */}
                        <div className="bg-warning/5 border border-warning/10 p-4 rounded-2xl relative overflow-hidden flex-1 group/bonus">
                            ...
                            <div className="relative z-10 space-y-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-caption font-black text-warning uppercase tracking-widest italic">{almanax.bonus.type?.name || "Bonus du jour"}</p>
                                </div>
                        ...
                        <Link href={`/dashboard/${guildId}/ressources?tab=almanax`} className="block text-center text-caption font-black text-muted-foreground hover:text-warning dark:hover:text-warning uppercase tracking-[0.2em] transition-colors py-2">
                            Voir le calendrier →
                        </Link>
                    </div>
                ) : (
