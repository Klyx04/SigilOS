        ...
        <Card className="glass-premium border-white/5 h-full flex flex-col overflow-hidden group">
            <CardHeader className="pb-4 pt-6 px-6 shrink-0">
                <div className="flex items-center justify-between">
                    ...
                    <Link href={`/dashboard/${guildId}/galerie-stuff`} className="text-[8px] font-black text-zinc-600 hover:text-emerald-400 uppercase tracking-widest border border-white/5 px-2 py-1 rounded-md transition-all">
                        Explorer la galerie
                    </Link>
                </div>
            </CardHeader>
            ...
            <CardContent className="flex-1 overflow-y-auto custom-scrollbar px-6 pb-6">
                {displayBuilds.length > 0 ? (
                        ...
                        {displayBuilds.map((build) => (
                            <div key={build.id} className="relative group/item scale-95 hover:scale-100 transition-all duration-300">
                                ...
                                <div className="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover/item:opacity-100 transition-opacity pointer-events-none z-20">
                                    <div className="px-2 py-1 bg-black/60 backdrop-blur-md rounded-lg border border-white/10 text-[8px] font-black text-emerald-400 uppercase tracking-widest shadow-2xl">
                                        {build.author.name}
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center py-20 text-zinc-600 italic border border-dashed border-white/5 rounded-[2rem]">
                        ...
                        <p className="text-[10px] uppercase font-black tracking-widest">Aucun stuff partagé</p>
                        <p className="text-[9px] mt-1 opacity-60">Partagez votre optimisation !</p>
                    </div>
                )}

                {displayBuilds.length > 0 && (
                    <div className="mt-8 flex justify-center">
                        <Link href={`/dashboard/${guildId}/galerie-stuff`} className="flex items-center gap-2 text-[10px] font-black text-zinc-600 hover:text-white uppercase tracking-widest group-hover:text-emerald-400 transition-colors">
