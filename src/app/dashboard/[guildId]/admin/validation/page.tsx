...
import { Gavel, ScrollText, Trophy } from "lucide-react";
...
export default async function UnifiedValidationPage({
    ...
    // Check for either mission validation or admin access
    ...
    const defaultTab = tab === "achievements" ? "achievements" : "missions";

    return (
        <div className="space-y-6 pb-12">
            <UnifiedModuleHeader
                title="Centre de Validation"
                description="Gérez toutes les preuves soumises par les membres (Missions et Succès)."
                icon={Gavel}
                iconColor="#f59e0b"
            ...
            <Tabs defaultValue={defaultTab} className="space-y-6">
                <TabsList className="bg-zinc-900/50 border border-white/5 p-1 h-12 rounded-xl">
                    <TabsTrigger value="missions" className="rounded-lg px-6 data-[state=active]:bg-blue-500/20 data-[state=active]:text-blue-400 gap-2">
                    ...
                    <TabsTrigger value="achievements" className="rounded-lg px-6 data-[state=active]:bg-amber-500/20 data-[state=active]:text-amber-400 gap-2">
