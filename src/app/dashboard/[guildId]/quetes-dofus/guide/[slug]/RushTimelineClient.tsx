...
type RushTimelineClientProps = { guide:{id:string;name:string;slug:string;description?:string|null;isUnderConstruction?:boolean;imageUrl?:string|null;isDiscordConfigured?:boolean}; milestones:Milestone[]; guildProgress:GuildMemberProgress[]; guildId:string; selectedCharacter?:string; mules?:any[]; currentUserProfile:{alignment?:string|null;alignmentOrder?:string|null;alignmentLevel?:number;altPseudos?:any[];dofusClass?:string|null;metamobPseudo?:string|null;pseudoDofus?:string|null;}; ocreStats?:{bosses?:{total:number;gathered:number};archis?:{total:number;gathered:number};progressPercent?:number;currentStep?:number;serverName?:string}|null; capturedOcreMonsterIds?:number[]; };
const DOFUS_DEFS = [
  ...
  return <div data-seq-id={seq.id} tabIndex={0} className={`flex items-start justify-between gap-3 px-3 py-2.5 rounded-xl border border-white/10 bg-zinc-900/60 hover:border-amber-500/30 hover:bg-zinc-900/80 transition-all cursor-default ${isSeqCompleted?"opacity-50":"shadow-sm"} ${focusedSeqId===seq.id?"ring-1 ring-amber-400/35":""}`}>
    <div className="flex items-start gap-2 min-w-0 flex-1">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-1 mt-1">
          ...
          {(()=>{const tags=seq.activityTags;const posTag=Array.isArray(tags)?(tags as any[]).find((t:any)=>t.type==="pos_tags"):null;if(!posTag?.name)return null;const posStr=String(posTag.name);const[copied,setCopied]=React.useState(false);return <span className="inline-flex items-center gap-1 px-2 py-1.5 rounded-full bg-gradient-to-r from-indigo-500/15 to-violet-500/15 border border-indigo-500/25 text-[10px] font-bold text-indigo-200 shadow-sm"><MapPin className="w-3.5 h-3.5 text-indigo-400"/><span className="font-mono text-[9px] tracking-wide">{posStr}</span><button onClick={e=>{e.stopPropagation();navigator.clipboard.writeText(`/travel ${posStr}`).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);}).catch(()=>{});}} className={`ml-0.5 px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border transition-all duration-200 ${copied?"bg-emerald-500/20 border-emerald-500/40 text-emerald-300 scale-95":"bg-indigo-500/20 border-indigo-500/30 text-indigo-200 hover:bg-indigo-500/30 hover:border-indigo-400/50 hover:scale-105 active:scale-95"}`} title="Copier /travel dans le presse-papier">{copied?<>✓ Copié</>:<><MapPin className="w-3 h-3 mr-1"/>Copier</>}</button></span>;})()}
    ...
    <div className="flex items-center gap-1.5 flex-shrink-0 mt-0.5">
      {(()=>{
        ...
        return <div className="flex items-center gap-1.5 flex-shrink-0">{djList.map((dj:any)=>{const captured=capturedMonsterSet&&capturedMonsterSet.size>0&&!!seq.metamobMonsterId&&capturedMonsterSet.has(seq.metamobMonsterId);return<button key={dj.id} onClick={e=>{e.stopPropagation();onDungeonClick(dj.id,questName);}} className={`flex items-center gap-1 px-2 py-1.5 rounded-xl border transition-all group ${captured?"bg-emerald-500/15 border-emerald-500/40 hover:bg-emerald-500/25":"bg-red-500/10 border-red-500/25 hover:bg-red-500/20"}`} title={`${dj.name}${captured?" ✓ Déjà capturé (Metamob)":""}`}>{dj.imageUrl?<img src={dj.imageUrl} alt={dj.bossName||dj.name} className={`w-7 h-7 rounded-lg object-cover border flex-shrink-0 ${captured?"border-emerald-500/50 ring-1 ring-emerald-500/30":"border-white/10"}`}/>:<div className="w-7 h-7 rounded-lg bg-zinc-800 border border-white/10 flex items-center justify-center"><Sword className="w-4 h-4 text-amber-400"/></div>}<span className={`text-[8px] font-black hidden sm:block flex items-center gap-1 ${captured?"text-emerald-300":"text-red-300"}`}>{dj.name}</span>{Array.isArray(seq.activityTags)&&seq.activityTags.some((t:any)=>t.type==="ocre_dungeon"&&t.name===dj.id)&&<span className="flex items-center gap-1 px-1.5 py-1 rounded-lg bg-amber-500/20 border border-amber-500/40 text-amber-300 text-[8px] font-black uppercase tracking-widest"><img src="/assets/icons/ocre.png" alt="" className="w-3.5 h-3.5 object-contain"/>Ocre</span>}{captured&&<span className="flex items-center gap-1 px-1.5 py-1 rounded-lg bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-[8px] font-black uppercase tracking-widest"><CheckCircle2 className="w-3 h-3"/>Capturé</span>}<Plus className={`w-3 h-3 group-hover:scale-125 transition-transform flex-shrink-0 ${captured?"text-emerald-400":"text-amber-400"}`}/></button>})}</div>;
      })()}
...
export default function RushTimelineClient({ guide, milestones, guildProgress, guildId, selectedCharacter="PRINCIPAL", mules=[], currentUserProfile, ocreStats, capturedOcreMonsterIds }: RushTimelineClientProps) {
  ...
  return(<CapturedMonsterCtx.Provider value={capturedMonsterSet}><style>{`footer,.site-footer,.app-footer,nav[class*="footer"]{display:none!important}`}</style>
  ...
  </div></CapturedMonsterCtx.Provider>);
}