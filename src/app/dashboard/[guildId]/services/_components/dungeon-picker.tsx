        ...
        <div ref={containerRef} className="relative space-y-3">
            ...
            <div className="relative">
                ...
                <Input
                    ...
                    disabled={disabled}
                    className="bg-surface border-border pl-9 pr-16"
                />
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                    {loading && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />}
                    {selectedDungeon && !loading && (
                        <button onClick={handleClear} className="text-muted-foreground hover:text-foreground transition-colors p-1">
            ...
            {/* Dropdown */}
            {open && (
                <div className="absolute z-50 w-full rounded-md border border-border bg-background shadow-xl max-h-64 overflow-auto">
                    ...
                    {results.map((dungeon) => (
                        ...
                        >
                            <div className="flex-1 min-w-0">
                                <p className="font-medium text-foreground truncate">{dungeon.name}</p>
                                <p className="text-xs text-muted-foreground truncate">Boss: {dungeon.bossName} • Niv. {dungeon.level}</p>
                            </div>
                            {dungeon.achievements.length > 0 && (
                                <Badge variant="outline" className="border-info/30 text-info text-caption flex-shrink-0">
            ...
            {selectedDungeon && selectedDungeon.achievements.length > 0 && (
                <div className="rounded-md border border-border bg-white/3 p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-1 gap-1.5">
                        {selectedDungeon.achievements.map((ach) => {
                            ...
                            return (
                                <div key={ach.id} className={`flex items-center gap-2.5 rounded-lg px-2 py-1.5 transition-colors cursor-pointer ${selectedAchievements.has(ach.id) ? "bg-info/10 border border-info/20" : "border border-transparent hover:bg-surface"
                                    ...
                                    <Checkbox
                                        ...
                                        className="border-border-strong data-[state=checked]:bg-info data-[state=checked]:border-info shrink-0"
                                    />
                                    {iconUrl && (
