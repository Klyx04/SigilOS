        ...
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg bg-white/5 p-3 text-center">
                    ...
                    <p className="text-2xl font-bold text-white">{songes.successRate}%</p>
                    <p className="text-xs text-zinc-500">Taux succès</p>
                </div>
                <div className="rounded-lg bg-white/5 p-3 text-center">
                    <TrendingUp className="w-5 h-5 text-amber-400 mx-auto mb-1" />
                    <p className="text-2xl font-bold text-white">{songes.avgFloor}</p>
                    <p className="text-xs text-zinc-500">Étage moyen</p>
                </div>
                <div className="rounded-lg bg-white/5 p-3 text-center">
                    <Users2 className="w-5 h-5 text-violet-400 mx-auto mb-1" />
                    <p className="text-2xl font-bold text-white">{songes.totalCandidatures}</p>
                    <p className="text-xs text-zinc-500">Candidatures</p>
                </div>
                <div className="rounded-lg bg-white/5 p-3 text-center">
                    <UserCheck className="w-5 h-5 text-teal-400 mx-auto mb-1" />
                    <p className="text-2xl font-bold text-white">{songes.acceptedCandidatures}</p>
            ...
            {songes.topLeaders.length > 0 && (
                <div>
                    <h4 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Top Leaders</h4>
                    <div className="space-y-1.5">
                        {songes.topLeaders.map((leader, i) => (
                            <div key={leader.name} className="flex items-center gap-2 text-sm">
                                ...
                                <span className="text-white font-medium truncate flex-1">{leader.name}</span>
                                <span className="text-emerald-400 font-bold">{leader.value}</span>
                            </div>
                        ))}
                    </div>
                </div>
