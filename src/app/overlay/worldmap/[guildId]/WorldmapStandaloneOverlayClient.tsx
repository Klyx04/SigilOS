...
interface WorldmapStandaloneOverlayClientProps {
  guildId: string;
  initialX?: number;
  initialY?: number;
  initialZoom?: number;
...
export function WorldmapStandaloneOverlayClient({
  guildId,
  initialX,
  initialY,
  initialZoom,
  initialWorldId,
}: WorldmapStandaloneOverlayClientProps) {
  ...
  return (
    <div className="w-screen h-screen flex flex-col bg-[#080a10] text-foreground font-sans select-none overflow-hidden isolate">
      {/* Header Overlay Compact & Élégant */}
      ...
      {/* Carte interactive plein écran autonome */}
