                    ...
                    <div className="space-y-12">
                        {Object.keys(groupedItems).length === 0 ? (
                            <div className="text-center py-16 bg-surface/30 border border-border rounded-xl">
                                <p className="text-muted-foreground text-lg">Aucun item sur la roadmap actuellement</p>
                            </div>
                        ) : (
                                            ...
                                            <div key={item.id} className="p-5 rounded-2xl bg-surface/60 border border-border hover:border-border transition-colors space-y-4">
                                                <div className="flex items-start justify-between">
                                                    ...
                                                    <div className="shrink-0 flex items-center">
                                                        {item.status === "DONE" && <Badge variant="outline" className="text-success border-success/30 bg-success/10 shadow-none"><CheckCircle2 className="w-3 h-3 mr-1" />Terminé</Badge>}
                                                        {item.status === "PROGRESS" && <Badge variant="outline" className="text-warning border-warning/30 bg-warning/10 shadow-none"><div className="w-2 h-2 rounded-full bg-warning mr-2 animate-pulse" />En cours</Badge>}
                                                        {item.status === "TODO" && <Badge variant="outline" className="text-muted-foreground border-border bg-elevated/50 shadow-none"><Circle className="w-3 h-3 mr-1" />À faire</Badge>}
                                                    </div>
                                                </div>
