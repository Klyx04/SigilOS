...
export function DofusTagManager() {
    ...
    return (
        <div className="space-y-4">
            {/* Header & Tag Library */}
            <div className="flex flex-col gap-6 bg-background/40 p-6 rounded-3xl border border-border backdrop-blur-md">
                <div className="flex items-center justify-between">
                   <div>
                        <h3 className="text-lg font-black text-foreground italic uppercase tracking-tighter flex items-center gap-2">
                            <Tag className="w-5 h-5 text-info" />
                            Bibliothèque & Création
                        </h3>
                        <p className="text-caption text-muted-foreground font-bold uppercase tracking-widest mt-1">Gérez vos tags et créez des coquilles vides</p>
                   </div>
                   <button 
                        onClick={() => setShowCreate(!showCreate)}
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl font-black uppercase text-caption tracking-widest transition-all ${showCreate ? 'bg-danger/10 text-danger border border-danger/20' : 'bg-info text-info-foreground hover:bg-info shadow-lg shadow-indigo-600/20'}`}
                   >
                        ...
                        >
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-surface p-4 rounded-2xl border border-border">
                                <div className="space-y-1">
                                    <label className="text-caption font-black uppercase text-muted-foreground tracking-widest ml-2">Nom du Dofus</label>
                                    <input 
                                        type="text" 
                                ...
                                <div className="space-y-1">
                                    <label className="text-caption font-black uppercase text-muted-foreground tracking-widest ml-2">Slug (URL)</label>
                                    <input 
                                        type="text" 
                                        placeholder="ex: forgelave"
                                        value={newItem.slug}
                                ...
                                <div className="space-y-1">
                                    <label className="text-caption font-black uppercase text-muted-foreground tracking-widest ml-2">Rareté</label>
                                    <input 
                                        type="text" 
                                        list="rarities"
                                ...
                                <div className="flex items-end pb-0.5">
                                    <button 
                                        ...
                                        className="w-full h-10 bg-success hover:bg-success disabled:opacity-30 disabled:hover:bg-success text-success-foreground font-black uppercase text-caption tracking-widest rounded-xl transition-all shadow-lg shadow-emerald-950"
                                    >
                ...
                <div className="flex flex-wrap gap-4 items-center">
                    <div className="flex items-center gap-3">
                        <span className="text-caption font-black uppercase text-muted-foreground tracking-widest">Vos Raretés :</span>
                        <div className="flex flex-wrap gap-2">
                            {uniqueRarities.map(r => (
                                <span key={r} className="px-2 py-0.5 bg-elevated border border-border rounded text-caption font-bold text-muted-foreground uppercase">{r}</span>
                            ))}
                        </div>
                    </div>
                    <div className="w-px h-4 bg-surface" />
                    <div className="flex items-center gap-3">
                        <span className="text-caption font-black uppercase text-muted-foreground tracking-widest">Vos Catégories :</span>
                        <div className="flex flex-wrap gap-2">
                            {uniqueCategories.map(c => (
                                <span key={c} className="px-2 py-0.5 bg-info/10 border border-info/20 rounded text-caption font-bold text-info uppercase">{c}</span>
                            ))}
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-wrap gap-3 pb-4 border-b border-border">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border bg-surface border-border">
                    <GripVertical className="w-3.5 h-3.5 text-muted-foreground" />
                    <span className="text-xs font-black uppercase tracking-widest text-muted-foreground">Ordre d'affichage</span>
                </div>
            </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-border bg-surface">
                                <th className="text-left px-5 py-3 text-caption font-black uppercase tracking-widest text-muted-foreground">Dofus</th>
                                <th className="text-center px-4 py-3 text-caption font-black uppercase tracking-widest text-muted-foreground">Rareté / Tag Principal</th>
                                {TAG_CONFIG.map((tag) => (
                                    <th key={tag.key} className="text-center px-4 py-3 text-caption font-black uppercase tracking-widest text-muted-foreground whitespace-nowrap">
                                        {tag.label}
                                    </th>
                                ))}
                                <th className="text-center px-4 py-3 text-caption font-black uppercase tracking-widest text-muted-foreground whitespace-nowrap">Catégorie Filtre</th>
                                <th className="text-center px-4 py-3 text-caption font-black uppercase tracking-widest text-muted-foreground">Ordre</th>
                                <th className="text-center px-4 py-3 text-caption font-black uppercase tracking-widest text-muted-foreground">Actions</th>
                            </tr>
                        </thead>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-5 py-3">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center border border-border bg-black/40">
                                                    {img ? (
                                                ...
                                                <div>
                                                    <p className="text-sm font-black text-foreground italic uppercase tracking-tight">{item.nameShort}</p>
                                                    <p className="text-caption text-muted-foreground font-medium">{item.slug}</p>
                                                </div>
                                                {isSaving && <Loader2 className="w-3.5 h-3.5 text-muted-foreground animate-spin ml-1" />}
                                            </div>
                                        </td>
                                                ...
                                                <td key={tag.key} className="text-center px-4 py-3">
                                                    <button
                                                        ...
                                                        className={`
                                                            ...
                                                            ${isActive
                                                                ? `${tag.activeBg} ${tag.activeBorder} ${tag.textColor} `
                                                                : "bg-surface border-border text-muted-foreground hover:text-muted-foreground hover:border-border-strong"
                                                            }
                                        ...
                                        <td className="text-center px-4 py-3">
                                            <div className="flex items-center justify-center gap-1">
                                                <button
                                                    ...
                                                    className="w-7 h-7 rounded-lg flex items-center justify-center bg-surface border border-border text-muted-foreground hover:text-foreground hover:border-border-strong transition-all disabled:opacity-20 cursor-pointer"
                                                >
                                                    <ArrowUp className="w-3.5 h-3.5" />
                                                </button>
                                                <span className="text-xs font-black text-muted-foreground w-6 text-center tabular-nums">{item.displayOrder}</span>
                                                <button
