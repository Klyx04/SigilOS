...
import { HardDrive, AlertTriangle, FileImage, Coins, Trophy, Image as ImageIcon, Building2 } from "lucide-react";
...
export async function GuildStorageCard({ guildId }: { guildId: string }) {
    ...
    return (
        <div className={cn(
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">
                            {d.totalFiles} fichier(s) · les captures sont nettoyées automatiquement (24h–7j)
                        </p>
                    </div>
                </div>
            ...
            {/* Répartition par type de capture */}
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                {d.breakdown.map((b) => {
                    const cfg = BREAKDOWN_ICON[b.key] ?? { icon: FileImage, color: "text-zinc-400" };
                    const Icon = cfg.icon;
                    return (
                        <div key={b.key} className="rounded-xl bg-black/30 border border-white/5 px-3 py-2.5 flex items-center gap-2.5">
