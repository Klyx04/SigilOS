...
function ResourceNameLink({ itemId, fallbackName }: { itemId: string; fallbackName?: string }) {
    ...
    return (
        <a 
            ...
            className="text-label font-black text-warning italic hover:text-warning transition-colors uppercase tracking-tight block truncate"
        >
    ...
    return (
        <div className="w-full relative z-10 isolate mb-12">
            <div 
                ...
                className={cn(
                    "w-full p-6 bg-surface border border-border rounded-[2.5rem] flex flex-col sm:flex-row items-center justify-between cursor-pointer hover:bg-surface transition-all group gap-6",
                    isExpanded && "rounded-b-none border-b-0 bg-surface"
                )}
            >
                <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8 w-full sm:w-auto">
                    <div className={cn(
                        ...
                        isExpanded ? "bg-background text-foreground border-border" : "bg-surface border-border text-muted-foreground"
                    )}>
                        <Info className="w-6 h-6" />
                    </div>
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-12 w-full text-center sm:text-left">
                        <div>
                            <div className="text-caption font-black text-muted-foreground uppercase tracking-widest mb-1">RÉCAPITULATIF GLOBAL</div>
                            <div className="text-[18px] font-black text-foreground italic uppercase tracking-tighter">Logistique du Dofus</div>
                        </div>
                        <div className="flex flex-wrap justify-center sm:justify-start items-center gap-4 sm:gap-8 text-caption font-black uppercase tracking-widest text-muted-foreground">
                ...
                <div className={cn(
                    "w-10 h-10 rounded-xl bg-surface flex items-center justify-center transition-transform shrink-0",
                    isExpanded && "rotate-180 bg-surface"
                )}>
            ...
            <AnimatePresence>
                {isExpanded && (
                    <motion.div
                        ...
                        className="overflow-hidden bg-surface border border-t-0 border-border rounded-b-[2.5rem]"
                    >
                        <div className="p-8 grid grid-cols-1 lg:grid-cols-12 gap-12">
                            {/* Resources Column */}
                            <div className="lg:col-span-5 space-y-6">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <h4 className="text-caption font-black text-muted-foreground uppercase tracking-widest flex items-center gap-3">
                                        <Package className="w-4 h-4" /> Ressources à prévoir
                                    </h4>
                                    <span className="text-caption font-black px-3 py-1 bg-surface rounded-full text-muted-foreground">{filteredItems.length} TYPES</span>
                                </div>

                                {isLoadingItems ? (
                                    <div className="p-8 border border-dashed border-border rounded-3xl text-center text-muted-foreground italic text-body-sm flex flex-col items-center gap-2">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                                        {filteredItems.map((item) => (
                                            <div key={item.id} className="flex items-center gap-3 p-3 bg-surface border border-border rounded-2xl hover:border-border hover:bg-surface transition-all group">
                                                <div className="w-10 h-10 bg-background rounded-xl flex items-center justify-center p-1.5 relative shrink-0 border border-border">
                                                    <img 
                                                    ...
                                                    />
                                                    <div className="absolute -top-1.5 -right-1.5 bg-warning text-warning-foreground text-caption font-black px-1.5 py-0.5 rounded-md shadow-lg border border-border">
                                                        x{item.amount}
                                                    </div>
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <ResourceNameLink itemId={item.id} fallbackName={item.name} />
                                                    <span className="text-caption font-bold text-muted-foreground uppercase tracking-widest block mt-0.5">ID: {item.id}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <div className="p-8 border border-dashed border-border rounded-3xl text-center text-muted-foreground italic text-body-sm">
                            ...
                            <div className="lg:col-span-7 space-y-6">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    ...
                                    <span className="text-caption font-black px-3 py-1 bg-surface rounded-full text-muted-foreground">{logistics.dungeons.length} BOSS</span>
                                </div>
                                        ...
                                        {logistics.dungeons.map((d, i) => {
                                            ...
                                            return (
                                                <a 
                                                    ...
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="flex flex-col gap-2 p-3 bg-danger/[0.02] border border-danger/10 rounded-2xl group hover:border-danger/30 hover:bg-danger/[0.05] transition-all relative overflow-hidden h-fit cursor-pointer"
                                                >
                                                    {/* Idol Badge */}
                                                    {d.idoleName && (
                                                        <div className="absolute top-0 right-0 px-2 py-0.5 bg-warning/20 border-b border-l border-warning/30 rounded-bl-lg text-caption font-black text-warning uppercase tracking-widest z-10">
                                                            {d.idoleName}
                                                        </div>
                                                    )}
                                                    
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 rounded-xl bg-black/60 border border-border flex items-center justify-center p-1 shrink-0 relative overflow-hidden group- transition-transform shadow-inner">
                                                            {(d.id || d.img) ? (
                                                                <img 
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <div className="text-caption font-black text-foreground italic truncate group-hover:text-danger transition-colors uppercase leading-tight">
                                                                {d.name}
                                                            </div>
                                                            <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-0.5 opacity-50">
                                                                {d.level && d.level > 0 && (
                                                                    <span className="text-caption text-muted-foreground font-bold uppercase tracking-wider whitespace-nowrap">Lvl {d.level}</span>
                                                                )}
                                                                {d.bossName && (
                                                                    <span className="text-caption text-muted-foreground font-bold italic truncate">— {d.bossName}</span>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </div>
