...
export function AchievementTracker({ guildId }: AchievementTrackerProps) {
    ...
    }, [dungeons, search, filterStatus, completedIds]);
                                ...
                                {isExpanded ? (
                                    ...
                                    <div className="bg-slate-900/90 border border-indigo-500/20 rounded-2xl overflow-hidden shadow-2xl shadow-indigo-900/10">
                                        ...
                                        >
                                            ...
                                            <div className="flex-1">
                                                <p className="font-black text-white">{dungeon.name}</p>
                                                <p className="text-xs text-slate-500">Lvl {dungeon.level} · {dungeon.bossName}</p>
                                            </div>
                                                ...
                                                return (
                                                    <div key={a.id}>
                                                        ...
                                                        >
                                                            ...
                                                            <span className="text-[10px] text-slate-600 shrink-0">{a.points}pts</span>
                                                        </div>
                                                        {!isDone && missingInfo && missingInfo.membersWhoHaveIt.length > 0 && (
                                                            <div className="ml-7 mt-1 flex items-center gap-1.5 flex-wrap">
                                                                <span className="text-[10px] text-slate-600">Déjà fait par :</span>
                                                                {missingInfo.membersWhoHaveIt.slice(0, 4).map((m: any, i: number) => (
                                                                    <span key={i} className="flex items-center gap-1 text-[10px] text-emerald-500 font-medium">
                                                                        {m.imageUrl && <img src={m.imageUrl} alt="" className="w-3.5 h-3.5 rounded-full" />}
                                                                        {m.name}
                                                                    </span>
                                                                ))}
                                                                {missingInfo.membersWhoHaveIt.length > 4 && (
                                                                    <span className="text-[10px] text-slate-600">+{missingInfo.membersWhoHaveIt.length - 4}</span>
                                                                )}
                                                            </div>
                                                        )}
                                                    </div>
