...
import {
    ...
    Trophy, Flame, Layers, Map, Link2, Calendar, LogIn,
    LogOut, Shield
} from "lucide-react";
import {
    acceptDjParticipant,
...
const MODE_LABELS: Record<string, string> = {
    FARM: "Farm", SUCCES: "Succès", MIXED: "Mixte", QUETE: "Quête",
};
...
export function DjPostDetailModal({
    ...
    const myParticipation = currentProfileId
        ...
        : null;
    const acceptedCount = post._acceptedCount + 1;
    ...
    function handleClose() {
        startTransition(async () => {
            ...
            if (res.success) {
                toast.success("Post fermé ! +5 points de contribution distribués.");
                onRefresh();
                onClose();
            } else toast.error(res.error);
        });
    ...
    function handleReject(participantId: string) {
        startTransition(async () => {
            ...
            if (res.success) { toast.success("Participant refusé."); onRefresh(); }
            else toast.error(res.error);
        });
    }
                ...
                <div className="space-y-5 pt-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="bg-slate-900/60 rounded-xl p-3 border border-slate-800">
                            ...
                            <p className="font-bold text-white">{MODE_LABELS[post.mode]}</p>
                        </div>
                        <div className="bg-slate-900/60 rounded-xl p-3 border border-slate-800">
                    ...
                    <div>
                        <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-2 flex items-center gap-2">
                            <Users className="w-3 h-3" /> Participants ({post.participants.filter(p => p.status !== "REJECTED").length})
                        </p>
                        <div className="space-y-2">
                            {/* Creator */}
                            <div className="flex items-center gap-3 bg-slate-900/40 rounded-xl p-2.5 border border-yellow-900/20">
                            ...
                            {/* Other participants */}
                            {post.participants.filter(p => p.status !== "REJECTED").map((p) => (
                                <div key={p.id} className="flex items-center gap-3 bg-slate-900/40 rounded-xl p-2.5 border border-slate-800">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold text-white truncate">
                                            {p.profile.pseudoDofus || p.profile.discordNickname || "Inconnu"}
                                        </p>
                                    ...
                                    {/* Creator actions */}
                                    {(isOwner || isAdmin) && p.status === "PENDING" && post.status === "OPEN" && (
                    ...
                    {/* Join form */}
                    {!isOwner && !myParticipation && post.status === "OPEN" && (
                    ...
                    {(isOwner || isAdmin) && post.status === "OPEN" && (
                        <div className="border-t border-slate-800 pt-4 flex gap-3">
                            {isOwner && (
                                <Button
                                    variant="outline"
                                    className="flex-1 border-emerald-900/40 text-emerald-400 hover:bg-emerald-500/10 font-bold h-10"
                                    onClick={handleClose}
                                    disabled={isPending}
                                >
