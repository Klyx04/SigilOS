...
function StatResist({
  ...
  return (
    <div className={`flex flex-col items-center gap-1 group/res relative`}>
      ...
      <span className="text-caption font-bold text-muted-foreground uppercase tracking-tighter">
        {label}
      </span>
    </div>
  );
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative w-full md:w-64">
                  ...
                  <input
                    ...
                    className="w-full bg-surface/50 border border-border rounded-xl pl-10 pr-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring/50 backdrop-blur-sm transition-all shadow-inner h-11"
                  />
                </div>

                <div className="flex flex-wrap items-center gap-1.5 p-1 bg-surface/40 border border-border rounded-xl">
                  {[
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-2">
                {filteredDungeons.map((dungeon) => (
                  <motion.button
                    ...
                    className="relative group overflow-hidden rounded-xl flex flex-col items-center text-center border border-border bg-surface/40 hover:bg-surface/60 hover:border-info/40 shadow-sm transition-all p-2 pb-2.5"
                  >
                    {/* Image */}
                    <div className="w-full aspect-square flex items-center justify-center mb-1.5 relative">
                      {dungeon.imageUrl ? (
                        ...
                        <div className="w-12 h-12 rounded-xl bg-surface flex items-center justify-center shadow-inner border border-border">
                          <Search className="w-5 h-5 text-muted-foreground" />
                        </div>
                      )}
                      {dungeon.isExpedition && (
                        <div className="absolute top-0 right-0 bg-info text-caption font-black px-1.5 py-0.5 rounded-bl-lg rounded-tr-lg shadow-lg uppercase tracking-tighter z-10">
                          Expédition
                        </div>
                      )}
                    </div>
                    ...
                    <div className="w-full">
                      <div className="inline-flex items-center justify-center px-1.5 py-0.5 rounded text-caption font-bold bg-info/10 text-info border border-info/20 mb-1">
                        Lvl {dungeon.level}
                      </div>
                      <h3 className="text-caption font-bold text-foreground leading-tight line-clamp-2 group-hover:text-foreground transition-colors">
                        {dungeon.name}
                      </h3>
                    </div>
                  </motion.button>
          ...
          >
            ...
            <div className="mb-2">
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-border bg-surface/50 text-foreground hover:bg-elevated hover:text-foreground transition-all shadow-sm"
              >
              ...
              <div className="relative z-10 p-6 md:p-8 flex flex-col md:flex-row items-start gap-6 lg:gap-8">
                ...
                <div className="flex-1 space-y-6 min-w-0">
                  <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-background border border-border rounded-lg shadow-inner">
                          <span className="text-caption font-black text-info uppercase tracking-widest">
                            Niveau {selectedDungeon.level}
                          </span>
                        </div>
                        {selectedDungeon.isExpedition && (
                          <div className="flex items-center gap-1.5 px-2 py-1 bg-success/10 border border-success/20 rounded-lg shadow-sm">
                            <Sword className="w-3 h-3 text-success" />
                            <span className="text-caption font-black text-success uppercase tracking-widest">
                              Expédition
                            </span>
                          </div>
                        )}
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-surface border border-border rounded-lg">
                      ...
                      <h2 className="text-2xl md:text-3xl font-black text-foreground drop-shadow-lg tracking-tight truncate">
                        {selectedDungeon.name}
                      </h2>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-2 pt-4 border-t border-border">
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Boss</span>
                          <span className="text-sm font-black text-foreground bg-surface px-2 py-1 rounded-lg border border-border">
                  ...
                  <div className="space-y-5">
                    ...
                    {bossStats?.drops && bossStats.drops.length > 0 && (
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <h4 className="text-caption font-black uppercase tracking-[0.2em] text-info/60 ml-1">
                            Butins notables
                          </h4>
                        </div>
                        <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto pr-2 pt-1.5 custom-scrollbar">
                          {bossStats.drops.map((drop: any, idx: number) => (
                            <button
                              ...
                              className="group/drop relative bg-surface border border-border rounded-md p-1 hover:bg-surface transition-all cursor-pointer flex items-center justify-center w-7 h-7 md:w-8 md:h-8 shrink-0"
                            ...
                            >
                              ...
                              />
                              <div className="absolute -top-1.5 -right-1 bg-info text-caption font-black px-1.5 py-0.5 rounded-sm shadow-xl opacity-0 group-hover/drop:opacity-100 transition-opacity whitespace-nowrap z-20 pointer-events-none origin-bottom-right">
                                {drop.percent}%
                              </div>
                            </button>
                          ))}
                    ...
                    {bossStats?.spells && bossStats.spells.length > 0 && (
                      <div className="space-y-2">
                        <h4 className="text-caption font-black uppercase tracking-[0.2em] text-danger/60 ml-1">
                          Capacités du Boss
                        </h4>
                            ...
                            return (
                              <button
                                ...
                                className={`flex items-center gap-2 border rounded-md px-1.5 py-1 transition-all outline-none group/spell ${
                                  isActive 
                                    ? "bg-danger/20 border-danger/40 text-danger shadow-lg" 
                                    : "bg-surface border-border text-muted-foreground hover:bg-surface hover:text-foreground"
                                }`}
                              >
                                <div className={`w-4 h-4 rounded overflow-hidden flex items-center justify-center p-0.5 transition-all ${isActive ? 'bg-danger/30' : 'bg-surface'}`}>
                                  {spell.imageUrl ? (
                                ...
                                <span className={`text-caption font-black uppercase tracking-tight ${isActive ? 'text-danger' : 'text-muted-foreground group-hover/spell:text-foreground'}`}>
                                  {spell.name}
                                </span>
                              </button>
                            );
                        ...
                        <AnimatePresence mode="wait">
                          {selectedSpellId && (
                            ...
                            >
                              <div className="bg-background/40 border border-border rounded-xl p-3 mt-1.5 space-y-2.5 shadow-inner">
                                {(() => {
                                    ...
                                    <>
                                      <div className="flex items-center items-start justify-between gap-3">
                                        <div className="flex items-center gap-2.5">
                                          <div className="w-7 h-7 rounded bg-black/40 flex items-center justify-center border border-border p-1">
                                            {spell.imageUrl ? <img src={spell.imageUrl} alt="" className="w-full h-full object-contain" /> : <Sword className="w-3.5 h-3.5 text-danger/50" />}
                                          </div>
                                          <div>
                                            <h5 className="text-xs font-black text-foreground">{spell.name}</h5>
                                            <div className="flex items-center gap-2 mt-0.5">
                                              <span className="text-caption font-black text-warning/70 bg-warning/5 px-1 py-0.5 rounded border border-warning/10 uppercase">{spell.apCost} PA</span>
                                              <span className="text-caption font-black text-info/70 bg-info/5 px-1 py-0.5 rounded border border-info/10 uppercase">{spell.minRange}-{spell.range} PO</span>
                                              <span className={`text-caption font-black px-1 py-0.5 rounded border uppercase ${spell.castTestLos ? 'text-success/70 bg-success/5 border-success/10' : 'text-danger/70 bg-danger/5 border-danger/10'}`}>
                ...
                <div className="md:w-56 lg:w-64 space-y-3 shrink-0">
                  ...
                  {bossStats?.coordinates && (
                    <div className="w-full h-60 rounded-2xl overflow-hidden border border-border  group/map relative bg-background">
                      <div className="absolute inset-x-0 bottom-0 z-10 pointer-events-none bg-gradient-to-t from-black/90 via-black/40 to-transparent flex items-end justify-between p-4">
                        <div className="flex items-center gap-2 text-caption font-black text-success uppercase tracking-widest italic shrink-0">
                  ...
                  {/* Stats Card */}
                  <div className="bg-background/30 border border-border rounded-xl p-3 shadow-xl">
                    <div className="flex items-center justify-between mb-2.5 border-b border-border pb-1.5">
                      <span className="text-caption font-black text-muted-foreground uppercase tracking-widest">Stats Boss</span>
                    ...
                    <div className="grid grid-cols-2 gap-1.5 text-caption">
                      <div className="bg-black/30 p-1.5 rounded-lg border border-border">
                        <span className="text-caption font-bold text-muted-foreground uppercase block">Vie</span>
                        <span className="text-foreground font-black">{bossStats?.grades[selectedGrade]?.lifePoints?.toLocaleString() || "---"}</span>
                      </div>
                      <div className="bg-black/30 p-1.5 rounded-lg border border-border">
                        <span className="text-caption font-bold text-muted-foreground uppercase block">PA | PM</span>
                        <span className="text-foreground font-black">{bossStats ? `${bossStats.grades[selectedGrade]?.actionPoints} | ${bossStats.grades[selectedGrade]?.movementPoints}` : "---"}</span>
                      </div>
                  ...
                  {onCreatePost && (
                    <Button
                      ...
                      className="w-full bg-info hover:bg-info text-info-foreground font-black py-3 rounded-lg shadow-lg shadow-indigo-950/20 transition-all hover:scale-[1.01] active:scale-95 text-caption uppercase tracking-wider"
                    >
            ...
            {/* Directory Content */}
            {loadingDir ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground animate-pulse bg-surface/30 rounded-3xl border border-border/50">
                <Search className="w-8 h-8 mb-3 opacity-50" />
                <p>Analyse des succès de la guilde...</p>
              </div>
            ) : filteredDirectory.length === 0 ? (
              <div className="bg-warning/10 border border-warning/20 rounded-2xl p-6 text-center text-warning/80">
                <Info className="w-8 h-8 mx-auto mb-2 opacity-60 text-warning" />
                ...
                <div className="flex items-center gap-2 ml-auto">
                  <div className="relative w-64">
                    ...
                    <input
                      ...
                      className="w-full bg-surface border border-border rounded-xl pl-10 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring/50 shadow-inner h-10 transition-all"
                    />
                  </div>
                  <button
                  ...
                  return (
                    <div
                      ...
                      className={`group/card rounded-2xl transition-all duration-300 overflow-hidden border ${
                        isExpanded
                          ? "bg-surface/90 border-info/30 "
                          : "bg-surface/30 border-border hover:border-white/12 hover:bg-surface/50 hover:shadow-lg shadow-sm"
                      }`}
                    >
                      <button
                        onClick={() =>
                      ...
                      >
                        <div className="flex items-center gap-4 flex-1 min-w-0">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border shadow-inner transition-all duration-300 ${
                            isExpanded
                              ...
                              : "bg-surface border-border group-hover/card:scale-105"
                          }`}>
                            {achv.iconUrl ? (
                              <img
                                src={achv.iconUrl}
                          ...
                          <div className="min-w-0">
                            <h4 className="text-base font-extrabold text-foreground leading-snug tracking-tight">
                              {achv.achievementName}
                            </h4>
                            <div className="flex items-center gap-2 mt-1">
                              {completionRate === 100 ? (
                                <span className="inline-flex items-center text-caption font-black uppercase tracking-wider px-2 py-0.5 rounded-md bg-success/10 text-success border border-success/20">
                                  Terminé
                                </span>
                              ) : completionRate > 0 ? (
                                <span className="inline-flex items-center text-caption font-black uppercase tracking-wider px-2 py-0.5 rounded-md bg-info/10 text-info border border-info/20">
                                  En Cours
                                </span>
                              ) : (
                                <span className="inline-flex items-center text-caption font-black uppercase tracking-wider px-2 py-0.5 rounded-md bg-elevated text-muted-foreground border border-border/50">
                                  Non Commencé
                                </span>
                              )}
                        ...
                        <div className="flex flex-col gap-1 w-full md:w-48 shrink-0">
                          <div className="flex items-center justify-between text-caption font-black uppercase tracking-wider text-muted-foreground">
                            <span>Progression</span>
                            <span className={completionRate === 100 ? "text-success font-extrabold" : "text-info font-extrabold"}>{completionRate}%</span>
                          </div>
                          <div className="w-full h-2 bg-black/40 rounded-full overflow-hidden border border-border shadow-inner relative">
                            <div
                        ...
                        <div className="flex items-center justify-between md:justify-end gap-4 shrink-0 mt-2 md:mt-0 border-t border-border pt-2.5 md:pt-0 md:border-none">
                          {/* Avatars Preview */}
                              ...
                              {achv.missing.slice(0, 4).map((m, i) => (
                                ...
                                >
                                  {m.imageUrl ? (
                                    ...
                                    <Users className="w-3 h-3 m-1.5 text-muted-foreground" />
                                  )}
                                </div>
                              ))}
                          ...
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                              isExpanded
                                ...
                                : "bg-surface text-muted-foreground border border-border hover:bg-surface"
                            }`}
                          >
                            <ChevronDown
                      ...
                      <AnimatePresence>
                        {isExpanded && (
                          ...
                          >
                            <div className="p-5 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 bg-background/20 shadow-inner">
                              {/* Col 1: MISSING */}
                              <div className="bg-background/40 border border-danger/10 rounded-2xl p-5 shadow-[inset_0_1px_4px_rgba(244,63,94,0.05)]">
                                <div className="flex items-center justify-between mb-4 pb-3 border-b border-border/60">
                                  <div className="flex items-center gap-2 font-bold text-sm">
                                    <div className="w-6 h-6 rounded-lg bg-danger/10 border border-danger/20 flex items-center justify-center">
                                      <Circle className="w-3.5 h-3.5 text-danger" />
                                    </div>
                                    <span className="text-danger">
                                      Cherchent encore
                                    </span>
                                  </div>
                                  <span className="text-xs font-black bg-danger/10 text-danger px-2 py-0.5 rounded-full border border-danger/20">
                                    {achv.missing.length}
                                  </span>
                                </div>
                              ...
                              {/* Col 2: HAS COMPLETED */}
                              <div className="bg-background/40 border border-success/10 rounded-2xl p-5 shadow-[inset_0_1px_4px_rgba(16,185,129,0.05)]">
                                <div className="flex items-center justify-between mb-4 pb-3 border-b border-border/60">
                                  <div className="flex items-center gap-2 font-bold text-sm">
                                    <div className="w-6 h-6 rounded-lg bg-success/10 border border-success/20 shadow-inner flex items-center justify-center">
                                  ...
                                  <span className="text-xs font-black bg-success/10 text-success px-2 py-0.5 rounded-full border border-success/20">
        ...
        {selectedDrop && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-[320px] bg-surface border border-border rounded-2xl overflow-hidden shadow-2xl"
            >
              <div className="p-6 flex flex-col items-center text-center space-y-4">
                <div className="w-20 h-20 rounded-2xl bg-black/40 border border-border flex items-center justify-center p-3 shadow-inner relative overflow-hidden group">
                   <div className="absolute inset-0 bg-gradient-to-br from-info/10 to-transparent opacity-50" />
                   <img src={selectedDrop.imageUrl} alt={selectedDrop.name} className="w-full h-full object-contain relative z-10 drop-shadow-md" />
                </div>
                
                <div>
                  <h3 className="text-lg font-black text-foreground leading-tight mb-1">{selectedDrop.name}</h3>
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-info/10 border border-info/20 text-info text-caption font-black uppercase tracking-wider">
                ...
                <div className="w-full grid grid-cols-2 gap-2 pt-2">
                  <button 
                    onClick={() => setSelectedDrop(null)}
                    className="px-4 py-2.5 rounded-xl border border-border bg-surface text-muted-foreground text-xs font-bold hover:bg-surface hover:text-foreground transition-all"
                  >
                    Fermer
                  </button>
                  <a 
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-4 py-2.5 rounded-xl bg-info hover:bg-info text-info-foreground text-xs font-black flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/20 transition-all active:scale-95"
                  >
  ...
  return (
    <div
      className={`group flex items-center justify-between p-2 rounded-xl border transition-all duration-200 ${
        isMissing
          ? "bg-background/40 hover:bg-surface/80 border-danger/10 hover:border-danger/30 text-danger shadow-[0_2px_8px_rgba(244,63,94,0.02)]"
          : "bg-success/20 hover:bg-success/30 border-success/10 hover:border-success/30 text-success shadow-[0_2px_8px_rgba(16,185,129,0.02)]"
      }`}
    >
      <div className="flex items-center gap-2.5 min-w-0">
        <div
          className={`w-7 h-7 rounded-lg overflow-hidden shrink-0 border transition-transform duration-200 group- ${
            isMissing
              ? "border-danger/20 bg-danger/30"
              : "border-success/20 bg-success/30"
        ...
        >
          {member.imageUrl ? (
            ...
            <Users className={`w-3.5 h-3.5 ${isMissing ? "text-danger/60" : "text-success/60"}`} />
          )}
        </div>
        ...
        <div className="flex flex-col min-w-0">
          ...
          {member.classe && (
            <span className="text-caption text-muted-foreground font-medium truncate capitalize">
              {member.classe}
            </span>
          )}
        </div>
