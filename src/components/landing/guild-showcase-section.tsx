...
function Stat({ icon: Icon, value, label }: { icon: typeof Users; value: string | number; label: string }) {
    return (
        <div className="flex items-center gap-2">
            ...
            <span className="text-body-sm font-bold text-white tabular-nums">{value}</span>
        ...
        <section className="py-20 relative w-full border-t border-white/5">
            <div className="container mx-auto px-6 relative z-10">
                <div className="max-w-4xl mx-auto text-center mb-14">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 mb-5 font-medium text-caption text-emerald-400 uppercase tracking-wider">
                        Communauté
                    </div>
                    <h2 className="text-3xl md:text-5xl font-heading text-foreground mb-4">
                        Des guildes qui <span className="text-emerald-400 font-bold">avancent ensemble.</span>
                    </h2>
                            ...
                            <div className="relative h-full rounded-2xl bg-zinc-900/40 border border-white/5 hover:border-emerald-500/20 hover:bg-zinc-900/60 transition-colors duration-150 overflow-hidden p-5 flex flex-col gap-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <p className="text-caption text-zinc-500 flex items-center gap-1.5 uppercase tracking-wider font-medium mt-0.5">
                                            <ShieldCheck className="w-3 h-3 text-emerald-500/70 shrink-0" />
                                            {guild.server || "Serveur Privé"}
                                        </p>
                                    </div>
                                    {guild.isRecruiting && (
                                        <span className="text-caption font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full uppercase tracking-wider shrink-0">
                                            Recrute
                                        </span>
                                    )}
                                </div>
