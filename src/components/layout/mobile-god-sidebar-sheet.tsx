...
export function MobileGodSidebarSheet({ user, unreadCount, ticketCount = 0, activeScopes = [], godRoute = "/god" }: { user: any, unreadCount: number, ticketCount?: number, activeScopes?: string[], godRoute?: string }) {
    ...
    return (
        <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="left" className="p-0 border-r border-white/10 bg-[#050505] w-[280px]">
                ...
                */}
                <GodSidebar user={user} unreadCount={unreadCount} ticketCount={ticketCount} activeScopes={activeScopes} godRoute={godRoute} className="w-full h-full flex" />
            </SheetContent>
        </Sheet>
    );
}
