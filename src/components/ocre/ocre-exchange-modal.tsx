...
import {
    ...
    getOcreExchangeJobStatus,
    createTradeRequest,
    getTradeAvailableDuplicatesAction,
...
export function OcreExchangeModal({ guildId, hasOcreChannel, trigger }: OcreExchangeModalProps) {
    ...
    const [myDuplicates, setMyDuplicates] = useState<DuplicateItem[]>([]);
    const [selectedOfferMonsterId, setSelectedOfferMonsterId] = useState<number | null>(null);
    ...
    // Load duplicates when initiating a trade request
    useEffect(() => {
        if (tradeRequest && guildId) {
            setLoadingDuplicates(true);
            getTradeAvailableDuplicatesAction(guildId)
                .then(res => {
                    if (res.success && res.data) {
                        setMyDuplicates(res.data);
                    }
                })
                .catch(() => {})
                .finally(() => setLoadingDuplicates(false));
        } else {
            setSelectedOfferMonsterId(null);
        }
    }, [tradeRequest, guildId]);
    ...
    const submitTradeRequest = async () => {
        ...
        try {
            const offeredMonster = myDuplicates.find(d => d.id === selectedOfferMonsterId);
            ...
            const res = await createTradeRequest({
                ...
                offeredMonsterId: offeredMonster ? offeredMonster.id : undefined,
                offeredMonsterName: offeredMonster ? offeredMonster.name : undefined,
                offeredMonsterImage: offeredMonster ? offeredMonster.imageUrl : undefined,
                offeredMonsterStep: offeredMonster ? offeredMonster.step : undefined,
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="w-[95vw] sm:max-w-4xl h-[90vh] sm:h-[85vh] !flex flex-col bg-surface border-border p-0 gap-0 overflow-hidden">
                <DialogHeader className="p-5 sm:p-6 border-b border-border flex-shrink-0">
                ...
                <div className="flex-1 overflow-hidden flex flex-col min-h-0">
                    {loading && partners.length === 0 ? (
                        ...
                        <div className="h-[400px] flex flex-col items-center justify-center gap-4 text-muted-foreground p-8 text-center">
                            ...
                            <p className="font-semibold text-foreground">Aucun échange trouvé pour le moment.</p>
                            <p className="text-sm opacity-60">
                                Assurez-vous que vos guildeux ont mis à jour leur compte Metamob !
                            </p>
                        </div>
                    ) : (
                        <Tabs defaultValue="monsters" className="h-full flex flex-col flex-1 min-h-0">
                            <div className="px-4 sm:px-6 pt-4 flex flex-col gap-3 flex-shrink-0">
                                {/* Responsive Tabs + Search */}
                                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                                    <TabsList className="flex items-center gap-1.5 bg-surface border border-border p-1 rounded-xl shrink-0 h-auto">
                                        <TabsTrigger 
                                            value="monsters" 
                                            className="rounded-lg font-bold px-3 py-1.5 text-xs uppercase tracking-wider flex items-center gap-2 transition-all text-muted-foreground hover:text-foreground data-[state=active]:bg-success data-[state=active]:text-success-foreground data-[state=active]:shadow-sm"
                                        >
                                            <PackageOpen className="h-3.5 w-3.5 shrink-0" />
                                            <span>Par Monstre</span>
                                            <Badge variant="secondary" className="px-1.5 py-0 h-4 text-[10px] font-black bg-black/20 text-inherit border-none">
                                                {monstersList.length}
                                            </Badge>
                                        </TabsTrigger>
                                        <TabsTrigger 
                                    ...
                                    <div className="relative flex-1 group">
                                        ...
                                        <Input
                                            ...
                                            placeholder="Rechercher un monstre ou archimonstre…"
                            ...
                            <TabsContent value="monsters" className="mt-4 focus-visible:outline-none flex-1 overflow-hidden flex flex-col min-h-0">
                                <div className="flex-1 overflow-y-auto px-4 sm:px-6 pb-6 min-h-0">
                                    {monstersList.length === 0 ? (
                                        <div className="h-[240px] flex flex-col items-center justify-center gap-3 text-muted-foreground text-center p-6">
                                            <PackageOpen className="h-10 w-10 opacity-30" />
                                            <p className="text-sm font-medium">Aucun monstre ne correspond aux filtres actuels.</p>
                                        </div>
                                    ) : (
                                                            ...
                                                            <div key={provider.name} className="flex items-center justify-between text-xs bg-background/80 border border-border p-1.5 rounded-lg">
                                                                <div className="flex items-center gap-2 min-w-0">
                                                                    <Link href={`/dashboard/${guildId}/members/${provider.profileId}`} target="_blank" rel="noopener noreferrer">
                                                                        <Avatar className="h-5 w-5 border border-border cursor-pointer hover:border-warning/50 transition-colors shrink-0">
                                                                    ...
                                                                    <div className="flex items-center truncate">
                                                                        <Link
                                                                            ...
                                                                            className="text-foreground hover:text-warning font-semibold transition-colors cursor-pointer truncate"
                                                                        >
                                                                            {provider.name}
                                                                        </Link>
                                                                        <span className="text-muted-foreground text-caption ml-1 font-normal shrink-0">(x{provider.count})</span>
                                                                    </div>
                                                                </div>
                                                                <div className="flex items-center gap-1 shrink-0">
                                                                    <Button
                                                                        variant="ghost"
                                                                        size="sm"
                                                                        className="h-7 px-2.5 gap-1 text-caption font-bold text-success hover:bg-success/10 hover:text-success rounded-lg"
                                                    ...
                                                    <div className="flex items-center justify-between mb-4">
                                                        <div className="flex items-center gap-3 min-w-0">
                                                            <Link href={`/dashboard/${guildId}/members/${encodeURIComponent(partner.slug || partner.profileId)}`} target="_blank" rel="noopener noreferrer">
                                                                <Avatar className="h-10 w-10 border border-border cursor-pointer hover:border-warning/50 transition-colors shrink-0">
                                                            ...
                                                            <div className="min-w-0">
                                                                <Link
                                                                    ...
                                                                    target="_blank"
                                                                    rel="noopener noreferrer"
                                                                    className="font-bold text-sm text-foreground hover:text-warning hover:underline cursor-pointer transition-colors truncate block"
                                                                >
                                                        ...
                                                        <span
                                                            ...
                                                            title="Score de correspondance Metamob"
                                                        >
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity ml-2 shrink-0">
                                                                    <Button
                                                                        variant="ghost"
                                                                        size="sm"
                                                                        className="h-7 px-2 gap-1 text-caption font-bold text-success hover:bg-success/10 hover:text-success rounded-lg"
                                                                        title="Envoyer une demande d'échange SigilOS"
                                                                        onClick={(e) => {
                                                                            e.stopPropagation();
                                                                            setTradeRequest({
            ...
            {/* Sub-Dialog for Trade Message with Troc Selector (#180) */}
            {tradeRequest && (
                <Dialog open={!!tradeRequest} onOpenChange={(open) => !open && setTradeRequest(null)}>
                    <DialogContent className="bg-background border-border sm:max-w-md max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Demande à <strong className="text-success">{tradeRequest.targetName}</strong> pour <strong className="text-success">{tradeRequest.monsterName}</strong>.
                            </DialogDescription>
                        </DialogHeader>

                        {/* Bilateral Trade / Troc Counterpart Selector */}
                        <div className="space-y-3 pt-2">
                            <Label className="text-xs font-bold uppercase tracking-wider text-foreground flex items-center gap-1.5">
                                <ArrowRightLeft className="w-3.5 h-3.5 text-warning" />
                                Proposer un monstre en contrepartie (Troc)
                            </Label>

                            {loadingDuplicates ? (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground p-3 bg-surface/50 border border-border rounded-xl">
                                    <Loader2 className="w-4 h-4 animate-spin text-warning" />
                                    Chargement de vos doublons disponibles…
                                </div>
                            ) : myDuplicates.length === 0 ? (
                                <div className="text-xs text-muted-foreground p-2.5 bg-surface/40 border border-border rounded-xl">
                                    Aucun doublon archimonstre détecté sur votre compte (échange unilatéral).
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    <select
                                        value={selectedOfferMonsterId || ""}
                                        onChange={(e) => setSelectedOfferMonsterId(e.target.value ? Number(e.target.value) : null)}
                                        className="w-full h-10 px-3 text-xs font-medium rounded-xl bg-surface border border-border text-foreground focus:outline-none focus:ring-1 focus:ring-success"
                                    >
                                        <option value="">Aucun (Demande simple / Don)</option>
                                        {myDuplicates.map(d => (
                                            <option key={d.id} value={d.id}>
                                                [Étape {d.step}] {d.name} (x{d.count} dispo)
                                            </option>
                                        ))}
                                    </select>
                                    {selectedOfferMonsterId && (() => {
                                        const picked = myDuplicates.find(d => d.id === selectedOfferMonsterId);
                                        if (!picked) return null;
                                        return (
                                            <div className="flex items-center gap-3 p-2 bg-warning/10 border border-warning/20 rounded-xl text-xs">
                                                {picked.imageUrl && (
                                                    <img src={picked.imageUrl} alt={picked.name} className="w-7 h-7 object-contain shrink-0" />
                                                )}
                                                <div className="flex-1 min-w-0">
                                                    <div className="font-bold text-foreground truncate">{picked.name}</div>
                                                    <div className="text-caption text-warning font-semibold">Étape {picked.step} · 1 exemplaire transféré</div>
                                                </div>
                                            </div>
                                        );
                                    })()}
                        ...
                        <div className="space-y-4 py-2">
                            <Textarea
                                placeholder="Message optionnel (ex: Dispo ce soir 20h zaap astrub ?)"
                                value={tradeMessage}
                                onChange={(e) => setTradeMessage(e.target.value)}
                                className="bg-black/50 border-border resize-none h-20 text-sm"
                                maxLength={500}
                            />
                                ...
                                <div className="space-y-0.5">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        {hasOcreChannel
                                            ...
                                            : "Configuration manquante sur le serveur."}
                                    </p>
                        ...
                        <DialogFooter className="gap-2 sm:gap-0">
                            ...
                            <Button onClick={submitTradeRequest} disabled={isSubmittingTrade} className="bg-success hover:bg-success text-success-foreground font-bold">
