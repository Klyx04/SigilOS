        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-12">
            ...
            <div className="rounded-[2.5rem] border border-border bg-surface p-8 relative overflow-hidden group">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <div>
                        <h3 className="text-caption font-black uppercase tracking-widest text-muted-foreground flex items-center gap-3 italic">
                            <div className="w-1.5 h-1.5 rounded-full bg-info " />
                            Engagement Discord
                        </h3>
                        <p className="text-muted-foreground text-xs mt-2 font-medium">Votre activité sur le serveur Discord de la guilde.</p>
                    </div>

                    <div className="flex p-1 bg-surface/50 rounded-2xl border border-border self-start">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-surface rounded-3xl p-6 border border-border flex items-center gap-6 group/stat">
                        <div className="p-4 rounded-2xl bg-info/10 text-info group-hover/stat:scale-110 transition-transform">
                            <Activity className="w-6 h-6" />
                        </div>
                        <div>
                            <div className="text-2xl font-black text-foreground tabular-nums tracking-tighter italic">
                                {currentDiscordStats.messages.toLocaleString()}
                            </div>
                            <div className="text-caption font-black uppercase text-muted-foreground tracking-widest mt-1">Messages Envoyés</div>
                        </div>
                    </div>
                    ...
                    <div className="bg-surface rounded-3xl p-6 border border-border flex items-center gap-6 group/stat">
                        ...
                        <div>
                            ...
                            <div className="text-caption font-black uppercase text-muted-foreground tracking-widest mt-1">Temps en Vocal</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="rounded-[2rem] border border-border bg-surface p-8 relative overflow-hidden group flex flex-col">
                    ...
                    {pieData.length === 0 ? (
                        <div className="flex-1 flex items-center justify-center text-muted-foreground text-caption uppercase font-bold italic tracking-widest bg-surface rounded-3xl border border-dashed border-border mt-4 min-h-[200px]">
                        ...
                        <>
                            <div className="h-[220px] relative mt-4">
                                ...
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
                                    <span className="text-3xl font-black text-foreground italic leading-none tracking-tighter">
                                        {pieData.reduce((acc, curr) => acc + curr.value, 0)}
                                    </span>
                                    <span className="text-caption text-muted-foreground uppercase font-black tracking-widest mt-1">Total</span>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-x-4 gap-y-3 mt-8">
                                {pieData.map((entry, i) => (
                                    <div key={entry.name} className="flex items-center gap-2">
                                        ...
                                        <span className="text-caption font-black text-muted-foreground uppercase italic truncate">{entry.name}</span>
                                        <span className="text-caption font-black text-foreground ml-auto">{entry.value}</span>
                                    </div>
                                ))}
                            </div>
                        </>
            ...
            {/* Bottom Note */}
            <div className="flex items-center justify-center gap-3 py-6 border-t border-border opacity-50">
                <BarChart3 className="w-4 h-4 text-muted-foreground" />
                <p className="text-caption font-black uppercase tracking-[0.2em] text-muted-foreground italic">
