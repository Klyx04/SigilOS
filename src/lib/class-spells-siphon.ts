/**
 * 🧙 **CŒUR du siphon des sorts de classes** (DofusDB `/breeds`, `/spell-variants`, `/spell-levels`).
 *
 * Descendu de `src/server/actions/dofus-spells-actions.ts` (24/09/2026) pour la **même raison** que
 * les autres cœurs : le worker BullMQ doit pouvoir remplir les 19 grimoires **sans session Next**
 * (les 14 classes vides ne pouvaient l'être qu'en gardant l'onglet ouvert).
 *
 * Invariants repris à l'identique :
 *   · forme **CANONIQUE** (grade niv. 200 + tous les grades) ⇒ stockable telle quelle ;
 *   · `/spell-levels` par lots de 40 (l'API plafonne à 50 lignes par page) ;
 *   · upsert idempotent par `classId` (jamais destructif) ;
 *   · icônes : `siphonAndCompressImage(..., 'spells', iconId)` (ignorées si déjà sur disque).
 */
...
import { SPELL_CHANGE_KEYS, spellDamageFromEffect, type SpellBaseDamage } from "@/lib/dofus-spells";
...
export const CLASS_SPELLS_TTL_MS = 24 * 60 * 60 * 1000;

/** Lecture du grimoire persisté (`ClassSpellbook`), `null` si absent/illisible. */
export async function readSpellbook(classId: number): Promise<{ spells: ClassSpellDamage[]; updatedAt: Date } | null> {
    try {
 ...
 * 3. `/spell-levels` → récupère tous les niveaux de sort par batchs de 40 (l'API plafonne à 50).
...
export async function fetchClassSpellsFull(classId: number): Promise<ClassSpellDamage[]> {
    ...
    try {
        ...
        const breed = breeds.find((b) => b?.shortName?.fr && normalizeLabel(b.shortName.fr) === target);
        if (!breed) return [];
        ...
        if (variants.length > 0) {
            ...
            rawSpells.forEach((s: any, idx: number) => {
                rawSpellsList.push({ raw: s, isVariant: false, variantPairId: idx + 1 });
                (Array.isArray(s?.spellLevels) ? s.spellLevels : []).forEach((id: any) => {
                    const n = Number(id);
                    if (Number.isFinite(n) && n > 0) allLevelIds.add(n);
                });
        ...
        // 3. Récupère tous les spellLevels par batchs de 40
...
/** Les 19 classes Dofus (ids Dofusbook) — même registre que l'écran God. */
export const CLASS_SPELLBOOK_CLASS_IDS: readonly number[] = Array.from({ length: 19 }, (_, i) => i + 1);

export interface ClassSpellbookSyncResult {
    classes: number;
    spells: number;
    grades: number;
    icons: number;
...
/**
 * 🔁 **Écriture unique** d'un grimoire : image *avant*, upsert idempotent, **journal** des
 * changements par sort. Deux portes appellent ce helper (la passe complète du worker et le bouton
 * « une classe » de l'écran God) ⇒ la règle d'écriture n'existe qu'**une fois**.
 */
export async function upsertClassSpellbookWithJournal(
    classId: number,
    className: string,
    ...
    await db.classSpellbook.upsert({
        where: { classId },
        create: { classId, className, spells: spells as unknown as object, spellCount: spells.length },
        update: { className, spells: spells as unknown as object, spellCount: spells.length },
    });

    const entries = diffCollection(
    ...
    await recordGameDataChanges("CLASS_SPELLS", entries);
    return { changed: entries.length };
}

/**
 * 🧙 **Passe complète des 19 grimoires** — exécutable **sans session Next** (worker BullMQ / cron).
 * C'est ce qui manquait : 14 classes restaient vides, faute de pouvoir lancer la boucle ailleurs que
 * dans un onglet ouvert (`warmClassSpellbook` est une action serveur, donc jamais lançable par le worker).
 *
 * Par classe : fetch → upsert idempotent → **journal des changements** (par sort : PA, portée, crit,
 * **dégâts**) → icônes WebP (best-effort : un échec d'icône ne fait pas échouer la classe).
 */
export async function siphonClassSpellbooksCore(
    onProgress?: (patch: { done: number; total: number; message?: string }) => void | Promise<void>,
    classIds: readonly number[] = CLASS_SPELLBOOK_CLASS_IDS,
): Promise<ClassSpellbookSyncResult> {
    const result: ClassSpellbookSyncResult = {
        classes: 0,
        spells: 0,
        grades: 0,
        icons: 0,
    ...
    for (let index = 0; index < classIds.length; index++) {
        ...
        try {
            ...
            if (full.length === 0) {
                result.errors.push(`${className} : aucun sort récupéré (DofusDB injoignable ?)`);
            } else {
                // Upsert + journal par **une seule porte** (`upsertClassSpellbookWithJournal`).
                await upsertClassSpellbookWithJournal(classId, className, full);

                result.classes++;
                result.spells += full.length;
                result.grades += full.reduce((total, spell) => total + (spell.grades?.length ?? 0), 0);

                // 🖼️ Icône de chaque sort (elle n'était résolue qu'**à chaud** sur DofusDB).
                for (const spell of full) {
                    if (!spell.imageUrl) continue;
                    try {
                        const icon = await siphonAndCompressImage(spell.imageUrl, "spells", spell.id);
                        if (icon.success) result.icons++;
                        else result.failedIcons++;
                    } catch {
                        result.failedIcons++;
                    }
                }
            ...
            result.errors.push(`${className} : ${message}`);
            logger.warn("[class-spells-siphon] classe en échec", { classId, error: message });
        }

        await onProgress?.({
            done: index + 1,
            total: classIds.length,
            message: `${className} (${index + 1}/${classIds.length})`,
        });
    }

    logger.info(
        `[class-spells-siphon] ${result.classes}/${classIds.length} grimoire(s) · ${result.spells} sort(s) · ${result.icons} icône(s) (${result.failedIcons} en échec)`,
    );
    return result;
}
