...
export type LadderType = "activity" | "seniority" | "achievements";
...
const UpdateAchievementPointsSchema = z.object({
    guildId: z.string(),
    points: z.number().int().min(0).max(100000), // Cap raisonnable pour éviter abus
});
...
/**
 * Get Achievements Ladder (Manual Dofus success points)
 */
export async function getAchievementsLadder(
    guildId: string
): Promise<ActionResponse<LadderEntry[]>> {
    try {
        const session = await auth();
        ...
        // Only include profiles with achievementPoints set
        const profiles = await db.userProfile.findMany({
            where: {
                guildId: guildConfig.id,
                status: "ACTIVE",
        ...
        const ladder: LadderEntry[] = profiles.map((p, idx) => ({
            ...
            pseudoDofus: p.pseudoDofus,
            classe: p.classe,
            value: p.achievementPoints || 0,
            isCurrentUser: p.id === currentProfile?.id
        }));

        return { success: true, data: ladder };
    } catch (error) {
        console.error("[getAchievementsLadder] Error:", error);
        return { success: false, error: "Erreur lors du chargement du classement" };
    }
}
...
/**
 * Update the current user's achievement points (manual entry)
 */
export async function updateMyAchievementPoints(
    rawData: z.infer<typeof UpdateAchievementPointsSchema>
    ...
    try {
        ...
        const data = UpdateAchievementPointsSchema.parse(rawData);
        ...
        console.error("[updateMyAchievementPoints] Error:", error);
